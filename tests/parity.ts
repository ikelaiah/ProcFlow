...
function exportEdgeMeaning(style: string, kind?: EdgeKind, dashed?: boolean): string {
  if(kind==='data') return 'data';
  if((style&&style==='dotted')||dashed) return 'dotted';
  return 'plain';
}
function exportManifest(graph: Graph) {
  return {
...
function clsFromClassDef(name: string): string {
...
function mermaidManifest(src: string) {
  var classes: Record<string, string>={};
  var dataLinks: Record<number, 1 | undefined>={};
  var nodeLines: Array<{id: string; rest: string}>=[];
  var edgeLines: Array<{from: string; to: string; label: string;
                        arrow: string; ordinal: number}>=[];
  var edgeOrdinal=0;
  src.split('\n').forEach(function(raw){
    var line=raw.trim();
    if(!line) return;
    if(/^classDef\s+\S+/.test(line)) return;
...
function drawioManifest(xml: string) {
  var nodes: Array<{id: string; text: string; cls: string; meta: string;
                    x: number; y: number; w: number; h: number}>=[];
  var edges: Array<{from: string; to: string; label: string;
                    meaning: string; kind: string;
                    waypoints: Array<{x: number; y: number}>}>=[];
  var parsererror: string | null=null;
  var doc=new DOMParser().parseFromString(xml,'application/xml');
  var pe=doc.querySelector('parsererror');
  if(pe) parsererror=(pe.textContent||'').slice(0,240);
  Array.prototype.forEach.call(doc.querySelectorAll('mxCell[vertex="1"]'),function(cell: Element){
  ...
  Array.prototype.forEach.call(doc.querySelectorAll('mxCell[edge="1"]'),function(cell: Element){
    var style=cell.getAttribute('style')||'';
    var dashed=/dashed=1/.test(style);
    var kind=cell.getAttribute('data-procflow-kind')||'';
    var waypoints: Array<{x: number; y: number}>= [];
    Array.prototype.forEach.call(cell.querySelectorAll('mxPoint'),function(p: Element){
    ...
    edges.push({
      ...
      meaning:exportEdgeMeaning('',(kind||undefined) as EdgeKind, dashed),
      kind:kind,
      waypoints:waypoints
    });
  });
...
var PROCFLOW_FILL_TO_CLS: Record<string, string>=(function(){
...
function clsAreEqual(a: string, b: string): boolean {
  if(a===b) return true;
  /* Legacy fallback: recover the class from the canonical fill registration
     when the metadata `cls=` attribute is absent. */
  if(a&&a.charAt(0)==='#') return (PROCFLOW_FILL_TO_CLS[a]||'')===b;
  return false;
}

/* ---------- parity fixture list (one per graph construct) ---------- */
var PROCFLOW_PARITY_FIXTURES: Array<{
  name: string; dialect: Dialect; sql: string; mode: DiagramMode;
  cls: 'control' | 'query' | 'data' | 'dependencies';
  nodeLimit: number; crossingBudget: number;
}>=[
...
var PROCFLOW_LAYOUT_CLASSES: Array<{
  cls: string; nodeLimit: number; crossingBudget: number; large: boolean;
}>=[
...
function parityGraphFor(fixture: typeof PROCFLOW_PARITY_FIXTURES[number]): Graph {
  return fixture.cls==='dependencies'
    ? analyseEstate([{name:fixture.name+'.sql', text:fixture.sql}],
        {dialect:fixture.dialect, mode:'auto', group:false, sources:true}).graph
    : analyse(fixture.sql, {dialect:fixture.dialect, mode:fixture.mode,
        group:true, sources:true, fanIn:true}).graph;
}
function parityFor(fixture: typeof PROCFLOW_PARITY_FIXTURES[number], dir: DiagramDirection) {
  var r=parityGraphFor(fixture);
  var expected=exportManifest(r);
  var expectedNode: Record<string, {text: string; cls: string}>={};
  ...
  var edgeSig=function(e: {from: string; to: string; label: string; meaning: string}): string {
    return e.from+'|'+e.to+'|'+e.label+'|'+e.meaning;
  };
  var mmSigns: Record<string, 1 | undefined>={};
  mm.edges.forEach(function(e){ mmSigns[edgeSig(e)]=1; });
  var expEdges=expected.edges.map(function(e){ return edgeSig(e); });
  var expSigns: Record<string, 1 | undefined>={};
  ...
  var dmNode: Record<string, {id: string; text: string; cls: string; meta: string}>={};
  ...
  var dmSigns: Record<string, 1 | undefined>={};
  ...
  r.nodes.forEach(function(n){
    ...
    if(!back) return;
    if(n.provenance) metaOk=metaOk&&back.meta.indexOf('provenance='+n.provenance)>=0;
    if(n.source) metaOk=metaOk&&back.meta.indexOf('span='+n.source.start+'-'+n.source.end)>=0;
    if(n.objectId) metaOk=metaOk&&back.meta.indexOf('object='+n.objectId)>=0;
    if(n.reason) metaOk=metaOk&&back.meta.indexOf('reason='+n.reason)>=0;
  });
...
function layoutFor(fixture: typeof PROCFLOW_PARITY_FIXTURES[number]) {
...
(function(){
  ...
  PROCFLOW_PARITY_FIXTURES.forEach(function(fixture){
    parityTotal+=2;
    (['TD','LR'] as DiagramDirection[]).forEach(function(dir){
      var p=parityFor(fixture, dir);
      if(p.ok) parityPassed++;
      else failures.push('parity '+fixture.name+' ['+dir+']: '+JSON.stringify(p.detail));
    });
    var graph=parityGraphFor(fixture);
  ...
  try{
    ...
    var bigOk=bigBig.graph.nodes.length<=bigCls.nodeLimit&&
      ...
      (bigA.crossings>bigCls.crossingBudget
        ...
        : bigA.warnings.length===0);
    if(bigOk) layoutPassed++; else failures.push('layout nonplanar: '+JSON.stringify({
  ...
  if(sum) sum.textContent='v1.7.0 exports · parity '+(parityPassed+'/'+parityTotal)+' · layout '+(layoutPassed+'/'+layoutTotal);
})();
