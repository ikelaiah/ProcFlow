...
(function(){
  ...
  interface ReportFixtureExpect {
    ...
    datasets?: Array<{
      ...
      hasAnalysis?: boolean;
      analysisReads?: string[];
    }>;
    diags?: string[];
    noDiags?: boolean;
    /* Codes that must be document-scoped with a null span (no fabricated
       source span). */
    docScoped?: string[];
    /* Codes that must be region-scoped with a valid span inside the source. */
    regionScoped?: string[];
    /* Every dataset and command-text span must fall inside the source and the
       source text at the span must contain the expected marker. */
    spansOk?: boolean;
  }
  interface ReportFixture {
    name: string;
    rdl: string;
    expect: ReportFixtureExpect;
  }
  ...
  function runReport(f: ReportFixture): ReportParseResult {
  ...
  PROCFLOW_REPORT_FIXTURES.forEach(function(f){
    try{
      ...
      var e=f.expect;
      var ok=true;
      var detail: any={reports:got.reports.map(function(r){
      ...
      if(e.reportCount!==undefined) ok=ok&&got.reportCount===e.reportCount;
      if(e.datasetCount!==undefined) ok=ok&&got.datasetCount===e.datasetCount;
      if(e.embeddedCount!==undefined) ok=ok&&got.embeddedCount===e.embeddedCount;
      if(e.sharedCount!==undefined) ok=ok&&got.sharedCount===e.sharedCount;
      if(e.unresolvedCount!==undefined) ok=ok&&got.unresolvedCount===e.unresolvedCount;
      ...
      if(e.diags) ok=ok&&(e.diags||[]).every(function(code){ return hasDiag(got.diagnostics,code); });
      if(e.noDiags){
  ...
  try{
    ...
    var ir=analyse('CREATE PROC dbo.report_proc AS BEGIN\n  SELECT 1;\nEND',
      {dialect:'tsql', mode:'flow', group:false, sources:true, reports:report});
    ...
    record('v1.12.0 analyse() attaches the report model and merges its diagnostics',
      integrated, JSON.stringify({
  ...
  try{
    ...
    record('v1.12.0 report diagnostics respect region/document scope and spans',
      spanless&&regionless, JSON.stringify({
  ...
  if(sum) sum.textContent='v1.12.0 reports · '+(passed+'/'+results.length);
})();