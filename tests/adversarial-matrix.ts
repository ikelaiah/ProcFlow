...
(function(){
  type Kind='node'|'edge'|'estate'|'estateEdge'|'diagnostic'|'opaque'|'columnRef'|'columnOutput';
  type Check={kind:Kind; must:'present'|'absent'; text?:string; from?:string;
    to?:string; edgeKind?:EdgeKind; relation?:'reads'|'writes'|'calls'; object?:string;
    code?:string; scope?:DiagnosticScope; severity?:DiagnosticSeverity;
    provenance?:NodeProvenance; resolution?:ColumnResolution; binding?:string; span?:boolean;};
  type Fixture={name:string; dialect:Dialect; sql:string; catalogue?:string; checks:Check[];};
  ...
  function same(a:string|undefined,b:string|undefined):boolean{return String(a||'').toUpperCase()===String(b||'').toUpperCase();}
  function spanOk(span:SourceSpan|null|undefined,sql:string):boolean{return !!span&&span.start>=0&&span.end>span.start&&span.end<=sql.length;}
  function matchedNodes(g:Graph,text:string):GraphNode[]{return g.nodes.filter(function(n){return n.text.indexOf(text)>=0;});}
  function label(c:Check):string{
    if(c.kind==='estate') return (c.relation||'relation')+' '+(c.object||'');
    if(c.kind==='estateEdge') return 'estate '+(c.edgeKind||'edge');
    if(c.kind==='edge') return (c.edgeKind||'edge')+' '+(c.from||'')+' -> '+(c.to||'');
    if(c.kind==='diagnostic') return 'diagnostic '+(c.code||'');
    return c.kind+' '+(c.text||'');
  }
  function check(c:Check,r:AnalysisResult,e:EstateResult,f:Fixture):{found:boolean;actual:unknown}{
    if(c.kind==='node'||c.kind==='opaque'){
      var ns=matchedNodes(r.graph,c.text||'').filter(function(n){return (c.kind!=='opaque'||n.cls==='opaque')&&
    ...
    if(c.kind==='estate'){
      var values:string[]=[];e.objects.forEach(function(o){values=values.concat((o[c.relation||'reads'] as string[])||[]);});
    ...
    var l=r.columns||[];
    if(c.kind==='columnRef'){
      var refs:ColumnReference[]=[];l.forEach(function(x){refs=refs.concat(x.references||[]);});
    ...
    var out:ColumnOutput[]=[];l.forEach(function(x){out=out.concat(x.outputs||[]);});
  ...
  var passed=0,total=0,required=0,forbidden=0,failures:Array<{dialect:Dialect;case:string;assertion:string;expected:string;actual:unknown}>=[];
  var byDialect:Record<string,{cases:number;assertions:number;required:number;forbidden:number}>={};
  fixtures.forEach(function(f){
    if(!byDialect[f.dialect]) byDialect[f.dialect]={cases:0,assertions:0,required:0,forbidden:0};
    byDialect[f.dialect].cases++;
    var cat=f.catalogue?parseCatalogue(f.catalogue):null;
    var opts:AnalyseOptions={dialect:f.dialect,mode:'flow',group:false,sources:true,fanIn:true,catalogue:cat&&cat.catalogue,catalogueDiagnostics:cat&&cat.diagnostics};
    var r=analyse(f.sql,opts),e=analyseEstate([{name:'matrix.sql',text:f.sql}],opts);
    f.checks.forEach(function(c){total++;byDialect[f.dialect].assertions++;if(c.must==='present'){required++;byDialect[f.dialect].required++;}else{forbidden++;byDialect[f.dialect].forbidden++;}
      var result=check(c,r,e,f),ok=c.must==='present'?result.found:!result.found;if(ok){passed++;return;}
  ...
  window.PROCFLOW_ADVERSARIAL_FAILURES=failures;window.PROCFLOW_ADVERSARIAL_PASS=passed===total;
})();
