...
(function(){
  ...
  interface ColFixtureExpect {
    ...
    outputCount?: number;
    wildcards?: Array<{source: string | null; expanded: boolean; columns?: string[]}>;
    refs?: Array<{text: string; resolution: ColumnResolution}>;
    sourceKeys?: string[];
    sourceOf?: {key: string; kind: ColumnSourceKind; columns?: string[]};
    diagnostics?: string[];
    noDiagnostics?: boolean;
  }
  interface ColFixture {
    name: string;
    dialect: Dialect;
    sql: string;
    catalogue?: string;
  ...
  function spanText(sql: string, span: SourceSpan | null): string {
  ...
  PROCFLOW_COLUMN_FIXTURES.forEach(function(f){
    try{
      ...
      if(!lin){ record(f.name+' · mappings',false,'column lineage was not produced'); return; }
      var e=f.expect, ok=true, detail: any={};
      ...
      if(e.sourceOf){
        var src: ColumnSource | null=null;
        lin.sources.forEach(function(s){ if(s.key.toUpperCase()===e.sourceOf!.key.toUpperCase()) src=s; });
      ...
      (e.outputs||[]).forEach(function(exp){
        var out: ColumnOutput | null=null;
        lin.outputs.forEach(function(o){ if(o.name===exp.name) out=o; });
      ...
      (e.refs||[]).forEach(function(exp){
        var ref: ColumnReference | null=null;
        lin.references.forEach(function(r){
          if(spanText(f.sql,r.span)===exp.text) ref=r;
        });
      ...
      (e.wildcards||[]).forEach(function(exp){
        var wc: ColumnWildcard | null=null;
        lin.wildcards.forEach(function(w){
          if((w.source||null)===(exp.source||null)) wc=w;
        });
  ...
  try{
    var r=analyse('SELECT id FROM dbo.a a JOIN dbo.b b ON a.id = b.aid;',
    ...
    record('v1.10.0 analyse() attaches column lineage and findings',
      integrated,
      JSON.stringify({columnCount:r.columns&&r.columns.length,
  ...
  try{
    ...
    record('v1.10.0 clean single-statement column bindings emit no diagnostics',
      cleanOk,
      JSON.stringify({diags:clean.diagnostics.map(function(d){return d.code;}),
  ...
  try{
    var probe=analyse('SELECT s.id FROM dbo.student s WHERE s.active = 1;',
      {dialect:'tsql',mode:'auto',group:false,sources:true});
    ...
    record('v1.10.0 output binding carries source, column, and span',
      provenanceOk,
      JSON.stringify(columns&&columns.outputs));
  }catch(err){
    record('v1.10.0 output binding carries source, column, and span',
      false,String(err&&err.stack||err));
  }
