...
(function(){
  ...
  function containsExecutableMarkup(text:string,doc?:Document):boolean {
    if(/<\s*(script|img|foreignObject)\b/i.test(text)) return true;
    if(!doc) return false;
    return Array.prototype.some.call(doc.querySelectorAll('*'),function(el:Element){
      for(var i=0;i<el.attributes.length;i++){
        var attr=el.attributes[i];
        if(/^(onerror|onload|onclick)$/i.test(attr.name)) return true;
        if(/^(href|xlink:href|src)$/i.test(attr.name)&&/^javascript:/i.test(attr.value)) return true;
      }
      return false;
    });
  }
  ...
  var catAnalysis=analyse('SELECT id FROM ['+payloads[0]+'];',
    {dialect:'tsql',mode:'query',sources:true,catalogue:catalogue.catalogue,
  ...
  record('hostile catalogue names stay data',
    !containsExecutableMarkup(catalogueMermaid)&&
      !containsExecutableMarkup(catalogueDrawio,xmlDoc),
    {diagnostics:catAnalysis.diagnostics.length,
  ...
  record('hostile RDL names do not become executable markup',
    parsedReport.reportCount===1&&parsedReport.datasetCount===1&&
      !containsExecutableMarkup(reportMermaid)&&!containsExecutableMarkup(reportXml,reportDoc)&&
      !reportDoc.querySelector('parsererror'),
    {reports:parsedReport.reportCount,datasets:parsedReport.datasetCount,
  ...
  record('source and diagnostic strings remain plain data',
    hostileSql.indexOf(payloads[0])>=0&&
  ...
  function finish(extra?:{name:string;pass:boolean;detail?:unknown}):void {
    ...
    if(summary) summary.textContent='v1.14.0 hostile-input security · '+passed+'/'+results.length;
  }
