...
(function(){
  ...
  function entityOf(result: SchemaResult, id: string): SchemaEntity | null {
    return result.entities.filter(function(entity){
      return entity.id===id.toUpperCase();
    })[0]||null;
  }
  function sqlFor(result: SchemaResult, selections: QueryColumnRef[],
                  extra?: Partial<QueryBuildInput>): string {
    var input: QueryBuildInput={result:result,selections:selections};
    if(extra) Object.keys(extra).forEach(function(key){
      (input as any)[key]=(extra as any)[key];
    });
    var plan=queryBuildPlan(input);
    return queryPlanSQL(plan,{dialect:'tsql',comments:false});
  }
  function planFor(result: SchemaResult, selections: QueryColumnRef[],
                   extra?: Partial<QueryBuildInput>): QueryPlan {
    var input: QueryBuildInput={result:result,selections:selections};
    if(extra) Object.keys(extra).forEach(function(key){
      (input as any)[key]=(extra as any)[key];
    });
  ...
  try{
    ...
    record('v2.4.0 join graph uses declared FKs and treats self-references as one edge',
      graph.order.length===11&&graph.edges.length===10&&graph.skipped.length===0&&
        ...
        customerEdge.childId==='DBO.CUSTOMER',
      {nodes:graph.order.length,edges:graph.edges.length});
    var heuristicEdge=graph.edges.filter(function(edge){
      return edge.resolution==='heuristic';
    })[0];
    record('v2.4.0 name-matched FK targets keep a higher path weight than exact ones',
      !!heuristicEdge&&heuristicEdge.weight===3&&
        graph.edges.filter(function(edge){
          return edge.resolution==='exact';
        }).every(function(edge){ return edge.weight===1; }),
      heuristicEdge);
  }catch(err){
  ...
  try{
    ...
    record('v2.4.0 single-table picks emit a plain SELECT with dialect quoting',
    ...
    record('v2.4.0 required child→parent FK joins with INNER and qualified conditions',
    ...
    record('v2.4.0 join explanations teach the direction and row effect',
      joinPlan.joins.length===1&&
    ...
    record('v2.4.0 nullable FKs join with LEFT in preserve mode and stay INNER in strict mode',
      optionalJoin.joins[0].joinType==='left'&&
        optionalJoin.joins[0].explanation.indexOf('nullable')>=0&&
        planFor(schema,[
          {entityId:'DBO.ORDERHEADER',column:'OrderId'},
          {entityId:'DBO.CAMPAIGN',column:'Name'}
        ],{policy:'strict'}).joins[0].joinType==='inner',
      optionalJoin.joins[0]);
    var selfPlan=planFor(schema,[
      {entityId:'DBO.CUSTOMER',column:'CustomerId'},
      {entityId:'DBO.CUSTOMER',column:'ReferredBy'}
    ]);
    record('v2.4.0 a self-referencing FK never invents a join',
      selfPlan.joins.length===0&&selfPlan.problems.length===0&&
        selfPlan.usedIds.length===1,
      selfPlan);
  }catch(err){
  ...
  try{
    ...
    record('v2.4.0 bridge tables are joined but never selected',
      bridge.joins.length===3&&
    ...
    record('v2.4.0 parent→child traversal keeps parent rows with LEFT and warns about row multiplication',
      reverseTypes==='left,left'&&
        ...
        sqlFor(schema,[
          ...
          'LEFT JOIN [dbo].[OrderHeader] AS orderheader')>=0,
      reverse);
    record('v2.4.0 strict policy turns preservation joins into INNER',
      planFor(schema,[
        {entityId:'DBO.CUSTOMER',column:'DisplayName'},
        {entityId:'DBO.ORDERLINE',column:'Quantity'}
  ...
  try{
    ...
    record('v2.4.0 equal-cost FK paths expose alternatives instead of hiding them',
      ambiguous.ambiguities.length===1&&
        ...
        ambiguous.joins[0].joinType==='inner',
      ambiguous);
    var chosen=planFor(schema,[
      {entityId:'DBO.ORDERHEADER',column:'OrderId'},
      {entityId:'DBO.APPUSER',column:'UserName'}
    ],{pathChoices:{'DBO.APPUSER->DBO.ORDERHEADER':1}});
    record('v2.4.0 choosing an alternative path changes the declared join and its row policy',
      chosen.joins[0].constraintName==='FK_Order_ApprovedBy'&&
        chosen.joins[0].joinType==='left'&&
        chosen.ambiguities[0].chosenIndex===1&&
        chosen.ambiguities[0].summary.indexOf('ApprovedBy')>=0,
      chosen.joins[0]);
    var note=planFor(schema,[
      {entityId:'DBO.NOTE',column:'OwnerId'},
      {entityId:'DBO.CUSTOMER',column:'Email'}
    ]);
    record('v2.4.0 name-matched FK paths are usable but warned about',
      note.joins[0].resolution==='heuristic'&&
        note.joins[0].joinType==='left'&&
        note.warnings.some(function(warning){
          return warning.indexOf('matched by name only')>=0;
        })&&
        note.joins[0].explanation.indexOf('name only')>=0,
      note);
  }catch(err){
  ...
  try{
    ...
    var disconnected=planFor(schema,selections,{comments:true} as any);
    record('v2.4.0 disconnected selections are reported as problems, not cartesian-joined',
      disconnected.joins.length===0&&
        ...
        disconnected.problems[0].education.indexOf('teach the join')>=0,
      disconnected.problems);
    record('v2.4.0 SQL never silently drops unjoined picks: it lists them in the header',
      sqlFor(schema,selections)===('SELECT\n  customer.[Email]\n'+
        'FROM [dbo].[Customer] AS customer;'),
      sqlFor(schema,selections));
    var plan=planFor(schema,selections);
    var withComments=queryPlanSQL(plan,{dialect:'tsql',comments:true});
    record('v2.4.0 provenance comments name the missing tables and the declaration-only rule',
      withComments.indexOf('/*')===0&&
        withComments.indexOf('declared FOREIGN KEY evidence only')>=0&&
        withComments.indexOf('Not included: dbo.AuditLog')>=0&&
        withComments.indexOf('Not included: dbo.Extra')>=0,
      withComments);
    ...
    record('v2.4.0 a taught join resolves the orphan and is labelled not declared',
      taught.problems.length===0&&
        ...
        sqlFor(schema,selections,{manual:[
          {id:'m1',leftId:'DBO.AUDITLOG',leftColumn:'EntityId',
           rightId:'DBO.CUSTOMER',rightColumn:'CustomerId'}
        ]}).indexOf('ON customer.[CustomerId] = auditlog.[EntityId]')>=0,
      taught);
    record('v2.4.0 a taught predicate is emitted verbatim',
      sqlFor(schema,selections,{manual:[
    ...
    record('v2.4.0 CROSS JOIN is an explicit opt-in and never overrides a declared path',
      cross.joins.length===1&&cross.joins[0].kind==='cross'&&
        sqlFor(schema,[
          {entityId:'DBO.CUSTOMER',column:'Email'},
          {entityId:'DBO.AUDITLOG',column:'EntityName'}
        ],{cross:['DBO.AUDITLOG']})
    ...
    record('v2.4.0 excluding a table removes its picks and its problem',
      excluded.problems.length===0&&excluded.selections.length===1&&
        excluded.usedIds.length===1,
      excluded);
    record('v2.4.0 a taught join pointing at an undeclared column is ignored with a warning',
  ...
  try{
    ...
    record('v2.4.0 composite FKs join every column pair with AND',
      composite.indexOf('shipment.[OrderId] = orderline.[OrderId] AND '+
        'shipment.[LineNo] = orderline.[LineNo]')>=0&&
        composite.indexOf('INNER JOIN [dbo].[OrderLine] AS orderline')>=0,
      composite);
    var duplicates=sqlFor(schema,[
      {entityId:'DBO.CUSTOMER',column:'Email'},
      {entityId:'DBO.APPUSER',column:'Email'}
    ]);
    record('v2.4.0 same-named columns from different tables get stable aliases',
      duplicates.indexOf('customer.[Email] AS customer_email')>=0&&
        duplicates.indexOf('appuser.[Email] AS appuser_email')>=0,
      duplicates);
    ...
    record('v2.4.0 same-named tables from different schemas get stable aliases',
      aliasSql.indexOf('FROM [a].[item] AS item')>=0&&
        aliasSql.indexOf('JOIN [b].[item] AS item_2')>=0,
      aliasSql);
    var view=planFor(schema,[
      {entityId:'DBO.CUSTOMER',column:'Email'},
      {entityId:'DBO.VORDERLINE',column:'Quantity'}
    ]);
    record('v2.4.0 views declare no keys and are reported instead of guessed',
      view.problems.length===1&&
        view.problems[0].message.indexOf('Views declare no keys')>=0,
      view.problems);
  ...
  try{
    ...
    record('v2.4.0 dialect quoting switches between brackets and double quotes',
      pgSql==='SELECT\n  customer."Email"\nFROM "dbo"."Customer" AS customer;'&&
        queryQuoteIdent('a]b','tsql')==='[a]]b]'&&
        queryQuoteIdent('a"b','postgres')==='"a""b"'&&
        queryQuoteIdent('a"b','db2')==='"a""b"',
      pgSql);
    var selections: QueryColumnRef[]=[
      {entityId:'DBO.ORDERLINE',column:'Quantity'},
      {entityId:'DBO.PRODUCT',column:'Sku'},
    ...
    record('v2.4.0 plans and SQL are deterministic',
      JSON.stringify(planA)===JSON.stringify(planB)&&
        queryPlanSQL(planA,{dialect:'tsql',comments:true})===
          queryPlanSQL(planB,{dialect:'tsql',comments:true}),
      planA);
    record('v2.4.0 an empty selection emits no SQL',
      queryPlanSQL(planFor(schema,[]),{dialect:'tsql',comments:true})===''&&
        queryBuildPlan({result:schema,selections:[]}).fromId===null,
      planFor(schema,[]));
    var highlightText=queryPlanSQL(planFor(schema,[
      {entityId:'DBO.CUSTOMER',column:'Email'}
    ]),{dialect:'tsql',comments:true});
    var tokens=queryTokenizeSQL(highlightText);
    record('v2.4.0 SQL highlighting tokens rebuild the exact SQL text',
      tokens.map(function(token){ return token.text; }).join('')===highlightText&&
        ...
        JSON.stringify(queryTokenizeSQL(highlightText))===JSON.stringify(tokens),
      {tokenCount:tokens.length});
    var predicateText="customer.[Email] = 'x' -- note\nAND n = 12";
    var predicateTokens=queryTokenizeSQL(predicateText);
    record('v2.4.0 SQL highlighting classifies strings, numbers, and line comments',
      predicateTokens.map(function(token){ return token.text; }).join('')===predicateText&&
  ...
  try{
    var ergoSelections: QueryColumnRef[]=[
      {entityId:'DBO.ORDERHEADER',column:'PlacedAt'},
      {entityId:'DBO.CUSTOMER',column:'Email'}
    ];
    ...
    record('v2.4.0 DISTINCT and the T-SQL TOP cap lead the SELECT list',
      tsql.indexOf('SELECT DISTINCT TOP 100\n')===0&&
        tsql.indexOf('ORDER BY orderheader.[PlacedAt] DESC;')>=0,
      tsql);
    ...
    record('v2.4.0 row caps and ORDER BY follow each dialect syntax',
      pg.indexOf('ORDER BY customer."Email" ASC\nLIMIT 50;')>=0&&
        pg.indexOf('SELECT\n')===0&&
      ...
      {pg:pg,lite:lite,db2:db2});
    var ignored=queryPlanSQL(ergoPlan,{dialect:'tsql',comments:false,
      orderBy:[{entityId:'DBO.CAMPAIGN',column:'Name',direction:'asc'}]});
    record('v2.4.0 ordering is limited to picked columns and cannot inject SQL',
      ignored.indexOf('ORDER BY')<0&&
        ignored.indexOf('Campaign')<0,
      ignored);
    var documented=queryPlanSQL(ergoPlan,{dialect:'tsql',comments:true,
      distinct:true,rowLimit:100,
      orderBy:[{entityId:'DBO.CUSTOMER',column:'Email',direction:'asc'}]});
    record('v2.4.0 ergonomics options are explained in the provenance header',
      documented.indexOf('Distinct: duplicate rows are collapsed.')>=0&&
        documented.indexOf('Row cap: first 100 rows only.')>=0&&
        documented.indexOf('Order: dbo.Customer.Email asc.')>=0,
      documented);
  }catch(err){
  ...
  try{
    var selfSelections: QueryColumnRef[]=[
      {entityId:'DBO.CUSTOMER',column:'DisplayName'},
      {entityId:'DBO.CUSTOMER',column:'ReferredBy'}
    ];
    var selfManual: QueryManualJoin[]=[
    ...
    record('v2.4.0 self joins add a second aliased copy and route copy columns',
      selfPlan.joins.length===1&&selfPlan.joins[0].selfJoin===true&&
        selfPlan.joins[0].copyColumns.join(',')==='DisplayName'&&
        selfSql===('SELECT\n  customer_2.[DisplayName],\n  customer.[ReferredBy]\n'+
          'FROM [dbo].[Customer] AS customer\n'+
          'INNER JOIN [dbo].[Customer] AS customer_2\n'+
          '  ON customer.[ReferredBy] = customer_2.[CustomerId];'),
      {sql:selfSql,joins:selfPlan.joins});
    var selfComments=queryPlanSQL(selfPlan,{dialect:'tsql',comments:true});
    record('v2.4.0 self joins are explained and labelled in the provenance header',
      selfComments.indexOf(
        'customer_2 = dbo.Customer (second copy, self join: DisplayName)')>=0&&
        selfPlan.education.some(function(line){
          return line.indexOf('self join')>=0;
        })&&
        selfPlan.joins[0].explanation.indexOf('Self join taught')>=0,
      selfComments);
    ...
    record('v2.4.0 a self join on an unpicked table is ignored with a warning',
      ignoredSelf.joins.length===0&&
    ...
    record('v2.4.0 only the first self join per table is applied',
      doubleSelf.joins.length===1&&
  ...
  try{
    ...
    for(var chainIndex=1;chainIndex<=1000;chainIndex++){
      chainLines.push('CREATE TABLE app.t'+chainIndex+' (',
        ...
        '  name VARCHAR(20),');
      if(chainIndex>1){
        chainLines.push('  ref_id INT NULL,',
          '  CONSTRAINT fk_qs'+chainIndex+' FOREIGN KEY (ref_id) REFERENCES app.t'+
            (chainIndex-1)+' (id),');
      }
    ...
    record('v2.4.0 the 1,000-table scale fixture parses',
      chainSchema.stats.tables===1000&&chainSchema.stats.relationships===999,
      chainSchema.stats);
    var chainSelections: QueryColumnRef[]=[
      {entityId:'APP.T1000',column:'name'},
      {entityId:'APP.T1',column:'name'}
    ];
    var chainPlan=planFor(chainSchema,chainSelections);
    record('v2.4.0 longest-path plans resolve without stack overflow',
      chainPlan.problems.length===0&&
        chainPlan.usedIds.length===1000&&
        chainPlan.joins.length===999&&
        chainPlan.bridges.length===998&&
        chainPlan.joins.every(function(join){ return join.joinType==='left'; }),
      {used:chainPlan.usedIds.length,joins:chainPlan.joins.length,
       bridges:chainPlan.bridges.length});
    var chainPlanAgain=planFor(chainSchema,chainSelections);
    record('v2.4.0 the 1,000-table plan is deterministic',
      JSON.stringify(chainPlan)===JSON.stringify(chainPlanAgain)&&
        queryPlanSQL(chainPlan,{dialect:'tsql',comments:false})===
          queryPlanSQL(chainPlanAgain,{dialect:'tsql',comments:false}),
      {joins:chainPlan.joins.length});
  }catch(err){
