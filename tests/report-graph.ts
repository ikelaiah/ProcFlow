...
(function(){
  ...
  interface ReportGraphFixtureExpect {
    reportNodes?: number;
    datasetNodes?: number;
    objectNodes?: number;
    columnNodes?: number;
    edgeCount?: number;
    /* Required edges between named nodes, e.g. [{from:'Students',to:'dbo.student',
       kind:'dependency'}] matching by node text. */
    required?: Array<{from: string; to: string; kind?: EdgeKind; label?: string}>;
    forbidden?: Array<{from: string; to: string}>;
    /* Report nodes must carry their XML span; dataset nodes their XML span;
       object nodes object identity (objectId) or a source span. */
    identityOk?: boolean;
    layout?: {nodeLimit: number; crossingBudget: number};
  }
  interface ReportGraphFixture {
    name: string;
    rdl: string;
    catalogue?: string;
    expect: ReportGraphFixtureExpect;
  }

  var RDL_NS='http'+'://schemas.microsoft.com/sqlserver/reporting/2008/01/reportdefinition';
  function rdl(head: string, body: string): string {
  ...
  function nodeByText(graph: Graph, text: string): GraphNode | null {
    var t=text.toLowerCase();
    var matches=(graph.nodes||[]).filter(function(n){ return n.text.toLowerCase().indexOf(t)>=0; });
    return matches.length?matches[0]:null;
  }
  function hasEdge(graph: Graph, from: string, to: string, kind?: EdgeKind, label?: string): boolean {
  ...
  var PROCFLOW_REPORTGRAPH_FIXTURES: ReportGraphFixture[] = [
    ...
    },
    {
      ...
      rdl:rdl('<DataSources></DataSources>',
        '<DataSets>\n'+
        '  <DataSet Name="Rollup">\n'+
        '    <SharedDataSet>\n'+
  ...
  function runGraph(f: ReportGraphFixture):
      {graph: Graph; report: ReportParseResult; catalogue: Catalogue | null} {
    var cat=null;
  ...
  PROCFLOW_REPORTGRAPH_FIXTURES.forEach(function(f){
    try{
      ...
      var e=f.expect;
      var ok=true;
      var detail: any={stats:graph.stats,
        nodes:graph.nodes.map(function(n){return n.cls+':'+n.text;}),
        edges:graph.edges.map(function(x){return x.from+'->'+x.to+'['+(x.kind||'')+'|'+(x.label||'')+']';})};

      if(e.reportNodes!==undefined) ok=ok&&graph.stats.reports===e.reportNodes;
      if(e.datasetNodes!==undefined) ok=ok&&graph.stats.datasets===e.datasetNodes;
      if(e.objectNodes!==undefined) ok=ok&&graph.stats.objects===e.objectNodes;
      if(e.columnNodes!==undefined) ok=ok&&graph.stats.columns===e.columnNodes;
      ...
      var expectedNode: Record<string, any>={};
      ...
      var edgeSig=function(x: any){ return x.from+'|'+x.to+'|'+x.label+'|'+x.meaning; };
  ...
  try{
    ...
    record('v1.13.0 report filtering is presentation-only (never mutates the graph)',
      presOk, JSON.stringify({full:full8.nodes.length,hiddenCols:hiddenCols.nodes.length,
  ...
  if(sum) sum.textContent='v1.13.0 reports · graph '+(passed+'/'+results.length)+
    ' · layout '+(layoutPassed+'/'+layoutTotal);
})();