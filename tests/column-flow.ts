...
(function(){
  ...
  function hasDiag(list: Diagnostic[], code: string): boolean {
    return (list||[]).some(function(d){return d.code===code;});
  }

  interface CFFixtureExpect {
    objectColumns?: Array<{
      object: string;
      columns: Array<{
        name: string;
    ...
    objectCount?: number;
    edges?: Array<{from: string; to: string; object: string;
                   resolution?: ColumnResolution; columns?: string[]}>;
    edgeCount?: number;
    stepCount?: number;
    diags?: string[];
    noDiags?: boolean;
    noEdgesExact?: boolean;
    graphNodes?: number;
    graphEdges?: number;
    dataEdges?: number;
    layout?: {nodeLimit: number; crossingBudget: number; label?: string};
  }
  interface CFFixture {
    name: string;
    dialect: Dialect;
  ...
  var PROCFLOW_COLUMNFLOW_FIXTURES: CFFixture[] = [
    ...
    /* ---- object boundaries ---- */
    {
      ...
      dialect:'tsql',
      estate:true,
      files:[{name:'x.sql', text:
        'CREATE VIEW dbo.v_students AS SELECT id, name FROM dbo.student;\n'+
        'GO\n'+
  ...
  function runCF(f: CFFixture): {cf: ColumnFlow | null; diags: Diagnostic[]} {
    var cat=null;
  ...
  PROCFLOW_COLUMNFLOW_FIXTURES.forEach(function(f){
    try{
      ...
      var ok=!!cf;
      var detail: any={};
      if(!cf){ record(f.name+' · column flow',false,'column flow was not produced'); return; }
      var e=f.expect;
      ...
      (e.objectColumns||[]).forEach(function(exp){
        ...
        var missing=!O;
        var colOk=true, colDetail: any={};
        exp.columns.forEach(function(c){
          var def: ColumnFlowOutput | null=null;
          (O&&O.columns||[]).forEach(function(x){ if(x.name===c.name) def=x; });
          var dOk=!!def&&
      ...
      if(e.edgeCount!==undefined) ok=ok&&cf.edges.length===e.edgeCount;
      ...
      (e.edges||[]).forEach(function(exp){
        ...
        if(exp.resolution==='exact'&&hit) hasExactEdge=true;
        ok=ok&&hit;
      });
      if(e.noEdgesExact){
      ...
      var spanOk=(got.diags||[]).every(function(d){
        if(d.code!=='column_flow_opaque') return true;
      ...
      if(e.graphNodes!==undefined) ok=ok&&graph.nodes.length===e.graphNodes;
      if(e.graphEdges!==undefined) ok=ok&&graph.edges.length===e.graphEdges;
  ...
  try{
    ...
    record('v1.11.0 analyse() attaches columnFlow, graph, and coverage',
  ...
  try{
    ...
    record('v1.11.0 clean temp pipeline carries no column-flow diagnostics',
      cleanOk,JSON.stringify({
  ...
  if(sum) sum.textContent='v1.11.0 columns · flow '+(passed+'/'+results.length)+
    ' · layout '+(layoutPassed+'/'+layoutTotal);
})();
