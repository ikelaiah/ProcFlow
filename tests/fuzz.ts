(function(){
  ...
  function mutate(sql: string): string {
    ...
    if(mode===1) return sql.slice(0,at)+sql.slice(Math.min(sql.length,at+1+Math.floor(random()*8)));
    if(mode===2){
      var len=1+Math.floor(random()*Math.min(20,sql.length-at||1));
      return sql.slice(0,at)+sql.slice(at,at+len)+sql.slice(at);
    }
    if(mode===3) return sql.slice(0,at);
    if(mode===4) return pick(inserts)+sql+pick(inserts);
  ...
  function graphInvariant(result: AnalysisResult, sql: string): boolean {
    ...
    result.graph.nodes.forEach(function(n){
      ...
      ids[n.id]=1;
      if(n.source&&(n.source.start<0||n.source.end<n.source.start||n.source.end>sql.length)) ok=false;
    });
  ...
  for(var i=0;i<cases;i++){
    ...
    try{
      ...
      if(!isFinite(a.coverage)||a.coverage<0||a.coverage>1)
        ...
        if(doc.querySelector('parsererror')) failures.push({case:i,reason:'draw.io XML malformed'});
      }
    }catch(err){
