...
(function(){
  ...
  var files: WorkspaceFile[]=[{name:'report.sql',text:src}];
  var optionsRecord: Record<string, unknown>={
  ...
  function optsFromSnapshot(snap: WorkspaceSnapshot): AnalyseOptions {
    var o=snap.options;
    return {dialect:o.dialect as DialectChoice, mode:o.view as DiagramMode,
      dir:o.dir as DiagramDirection, detail:o.detail as any,
  ...
  try{
    ...
    record('v1.8.0 snapshot round-trip preserves every field',
      parsed.snapshot!==null&&!parsed.error&&parsed.migrated===false&&
        ...
        parsed.snapshot.files[0].text===snap.files[0].text,
      {serialized:serialized});

    record('v1.8.0 save → reload reproduces an identical analysis',
      (function(){
      ...
      {graph:(parsed.snapshot?graphKey(analyseEstate(parsed.snapshot.files,
        optsFromSnapshot(parsed.snapshot)).graph):''),
       graph2:(parsed.snapshot?graphKey(analyseEstate(files,
         optsFromSnapshot(snap)).graph):'')});
  }catch(err){
    record('v1.8.0 snapshot round-trip preserves every field',
  ...
  try{
    ...
    record('v1.8.0 older-version snapshots migrate forward with defaults',
      !!legacy.snapshot&&legacy.migrated===true&&legacy.snapshot.version===WORKSPACE_SCHEMA_VERSION&&
        legacy.snapshot.options.view==='auto'&&
        legacy.snapshot.options.group===true&&
        legacy.snapshot.options.sources===true&&
        legacy.snapshot.files[0].name==='old.sql',
      legacy);
    record('v1.8.0 current-version snapshots are not migrated',
      (function(){
        var cur=parseWorkspace(serializeWorkspace(
  ...
  try{
    ...
    record('v1.8.0 corrupt-state recovery returns an error instead of throwing',
      corruptJson.error==='corrupt_json'&&corruptJson.snapshot===null&&
        corruptType.error==='not_workspace'&&corruptType.snapshot===null&&
        corruptEmpty.error==='empty_workspace'&&corruptEmpty.snapshot===null,
      {corruptJson:corruptJson,corruptType:corruptType,corruptEmpty:corruptEmpty});
  }catch(err){
    record('v1.8.0 corrupt-state recovery returns an error instead of throwing',
      false,String(err&&err.stack||err));
  }
  ...
  try{
    ...
    try { var probe=window.localStorage.getItem('procflow.probe'); }
    ...
    if(storageAvailable){
      ...
      record('v1.8.0 opt-in storage round-trip and explicit clear',
        wrote===true&&before===false&&stored===true&&
  ...
  try{
    ...
    record('v1.8.0 dependency filter default shows the whole graph',
      allOn.nodes.length===4&&allOn.edges.length===4,
      {nodes:allOn.nodes.length,edges:allOn.edges.length});
    record('v1.8.0 dependency filter hides write edges without dropping nodes',
      noWrites.edges.length===3&&
        !noWrites.edges.some(function(e){return e.kind==='data';})&&
        noWrites.nodes.length===4,
      {nodes:noWrites.nodes.length,edges:noWrites.edges.length,
       kinds:noWrites.edges.map(function(e){return e.kind;})});
    record('v1.8.0 dependency filter drops external nodes and their incident edges',
      noExternal.nodes.length===3&&noExternal.edges.length===2&&
        !noExternal.nodes.some(function(n){return n.provenance==='external';}),
      {nodes:noExternal.nodes.length,
       edges:noExternal.edges.map(function(e){return e.kind;})});
    record('v1.8.0 dependency filter drops temp-table nodes and their incident edges',
      noTemp.nodes.length===3&&noTemp.edges.length===2&&
        !noTemp.nodes.some(function(n){return n.provenance==='synthetic';}),
      {nodes:noTemp.nodes.length,
       edges:noTemp.edges.map(function(e){return e.kind;})});
    record('v1.8.0 dependency filter focus keeps the matching neighbourhood',
      (function(){
    ...
    record('v1.8.0 a focus matching nothing shows an empty view',
      noneMatch.nodes.length===0&&noneMatch.edges.length===0,
      {nodes:noneMatch.nodes.length,edges:noneMatch.edges.length});
    record('v1.8.0 dependency filtering never mutates the analysis graph',
      JSON.stringify(baseline)===JSON.stringify(depGraph)&&
        allOn.nodes!==depGraph.nodes&&allOn.edges!==depGraph.edges,
      {unchanged:JSON.stringify(baseline)===JSON.stringify(depGraph),
