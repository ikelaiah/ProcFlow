...
(function(){
  ...
  try{
    ...
    record('v1.9.0 JSON catalogue parses objects, columns, synonyms, and kind',
      jp.format==='json'&&!jp.diagnostics.some(function(d){return d.severity==='error';})&&
        ...
        resolveCatalogue(jp.catalogue,'dbo.nope').resolution==='external',
      {format:jp.format,diagnostics:jp.diagnostics,resolved:
        resolveCatalogue(jp.catalogue,'student')});
  }catch(err){
    record('v1.9.0 JSON catalogue parses objects, columns, synonyms, and kind',
  ...
  try{
    ...
    record('v1.9.0 line catalogue parses objects, kinds, synonyms, and columns',
      tp.format==='text'&&tp.objectCount===4&&tp.columnCount===1&&
        ...
        resolveCatalogue(tp.catalogue,'remotesrv.salesdb.dbo.pull_orders')
          .resolution==='verified',
      {format:tp.format,objects:tp.objectCount,columns:tp.columnCount,
  ...
  try{
    ...
    record('v1.9.0 malformed catalogue JSON reports catalogue_parse_error',
      badJson.format==='json'&&badJson.objectCount===0&&
  ...
  try{
    ...
    record('v1.9.0 empty catalogue input reports catalogue_empty',
      emptyC.objectCount===0&&emptyC.diagnostics.some(function(d){
  ...
  try{
    ...
    var dupRegion=false;
    var dupAnalysis=analyse('SELECT id FROM dbo.orders;',
      {dialect:'tsql',mode:'query',sources:true,
    ...
    record('v1.9.0 duplicate catalogue entries are conservative with a conflict diagnostic',
      dup.diagnostics.some(function(d){return d.code==='catalogue_conflict';})&&
  ...
  try{
    ...
    record('v1.9.0 a synonym colliding with an object name is a conflict',
      coll.diagnostics.some(function(d){return d.code==='catalogue_conflict';})&&
        resolveCatalogue(coll.catalogue,'alias').resolution==='conflict',
  ...
  try{
    ...
    var est=analyseEstate([{name:'est.sql',text:estSql}],
    ...
    record('v1.9.0 estate external nodes verified by the catalogue',
      has(verifiedTexts,'remotesrv.salesdb.dbo.pull_orders')&&
  ...
  try{
    ...
    var qr=analyse('SELECT ord.id FROM linksrv.warehouse.dbo.orders ord WHERE ord.id > 0;',
      {dialect:'tsql',mode:'query',sources:true,
       catalogue:qc.catalogue,catalogueDiagnostics:qc.diagnostics});
    var qNode=qr.graph.nodes.find(function(n){return n.cls==='src';});
    record('v1.9.0 query sources verified by the catalogue',
      !!qNode&&qNode.text==='linksrv.warehouse.dbo.orders'&&
        qNode.resolution==='verified'&&qNode.provenance==='external'&&
        qNode.resolvedName==='linksrv.warehouse.dbo.orders',
      qNode);
  }catch(err){
    record('v1.9.0 query sources verified by the catalogue',
      false,String(err&&err.stack||err));
  ...
  try{
    ...
    var partialR=analyse('SELECT id FROM remotesrv.salesdb.dbo.orders;',
      {dialect:'tsql',mode:'query',sources:true,
    ...
    var partialDiag=partialR.diagnostics.filter(function(d){
      return d.code==='catalogue_partial';})[0];
    record('v1.9.0 unproven cross-database matches stay external and region-scoped',
      !!partialNode&&partialNode.text==='remotesrv.salesdb.dbo.orders'&&
        ...
        partialDiag.message.indexOf('salesdb.dbo.orders')>=0,
      {node:partialNode,diag:partialDiag});
  }catch(err){
    record('v1.9.0 unproven cross-database matches stay external and region-scoped',
      false,String(err&&err.stack||err));
  }
  ...
  try{
    var nonC=analyse('SELECT id FROM linksrv.warehouse.dbo.orders;',
      {dialect:'tsql',mode:'query',sources:true});
    var nonNode=nonC.graph.nodes.find(function(n){return n.cls==='src';});
    record('v1.9.0 absent catalogue leaves the conservative path unchanged',
      !!nonNode&&nonNode.text==='linksrv.warehouse.dbo.orders'&&
  ...
  try{
    ...
    var synEst=analyseEstate([{name:'syn.sql',text:synSql}],
      {dialect:'tsql',mode:'auto',catalogue:synC.catalogue,
    ...
    record('v1.9.0 an unqualified synonym resolves to its canonical object',
      synEst.graph.nodes.some(function(n){
        return n.text==='dbo.student'&&n.resolution==='verified'&&
          n.resolvedName==='dbo.student'&&n.objectId==='student';
      })&&synLeaf.every(function(n){ return n.resolution==='verified'; }),
      synEst.graph.nodes.map(function(n){
  ...
  try{
    ...
    var eRef=analyse('SELECT id FROM salesdb.dbo.orders;',
      {dialect:'tsql',mode:'query',sources:true,
    ...
    record('v1.9.0 catalogue resolution and provenance survive draw.io export',
      !eDoc.querySelector('parsererror')&&
        eVerified.length>0&&
        eXml.indexOf('resolution=verified')>=0&&
        eXml.indexOf('provenance=external')>=0,
      {parsererror:eDoc.querySelector('parsererror')&&
  ...
  try{
    ...
    record('v1.9.0 workspace snapshot round-trips the catalogue text',
      wsCatalogueRoundTrip&&wsLegacyOk,
      {roundTrip:wsCatalogueRoundTrip,legacy:wsLegacyOk,
