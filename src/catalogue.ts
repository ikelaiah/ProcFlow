/* ===== v1.9.0 Resolve by catalogue (README post-v1.0.0 item 5) =====
   ...
   later release. */
var CATALOGUE_KINDS: StringSet = S(['TABLE','VIEW','PROC','PROCEDURE','FUNCTION',
...
function normalizeCatalogueKind(kind: unknown): CatalogueKind {
  var k=String(kind==null?'':kind).trim().toUpperCase();
  if(k==='PROCEDURE') return 'PROC';
  if(['TABLE','VIEW','PROC','FUNCTION','TRIGGER','SYNONYM','TYPE','SEQUENCE']
       .indexOf(k)>=0) return k as CatalogueKind;
  return 'OTHER';
}
...
function buildCatalogueIndex(objects: CatalogueObject[], columns: CatalogueColumn[]): Catalogue {
  var byName: Record<string, CatalogueObject>={},
...
function resolveCatalogue(cat: Catalogue | null | undefined,
    name: string): {resolution: CatalogueResolution; resolvedName?: string} {
  if(!cat||!(cat.objects&&cat.objects.length)) return {resolution:'external'};
  var n=normalizeCatalogueName(name);
  if(!n) return {resolution:'external'};
  if(cat.conflicts[n]===1) return {resolution:'conflict'};
  var exact=cat.byName[n];
  if(exact) return {resolution:'verified', resolvedName:exact.name};
  var syn=cat.bySynonym[n];
  if(syn) return {resolution:'verified', resolvedName:syn.name};
...
function suffixCatalogueMatches(cat: Catalogue | null | undefined,
    name: string): string[] {
  if(!cat||!name) return [];
  var n=normalizeCatalogueName(name), out: string[]=[], seen: StringSet={};
  (cat.objects||[]).forEach(function(o){
    var on=normalizeCatalogueName(o.name);
    if(!on||on===n) return;
    if(on.length>=n.length) return;
    if(n.charAt(n.length-on.length-1)==='.'&&
       n.slice(n.length-on.length)===on){
      var key='#'+o.name.toUpperCase()+':'+o.kind;
      if(!seen[key]){ seen[key]=1; out.push(o.name); }
    }
  });
...
function parseCatalogue(text: string, format?: 'json' | 'text'): CatalogueParseResult {
  var src=String(text==null?'':text);
  var trimmed=src.trim();
  var fmt: 'json' | 'text' = format||
    ((trimmed.charAt(0)==='{'||trimmed.charAt(0)==='[') ? 'json' : 'text');
  var diagnostics: Diagnostic[]=[];
  var objects: CatalogueObject[]=[], columns: CatalogueColumn[]=[];
  ...
  if(fmt==='json'){
    ...
    var objSeen: StringSet={};
    (list||[]).forEach(function(entry: any){
      if(typeof entry==='string'){ objects.push({name:entry,kind:'OTHER',synonyms:[]}); return; }
      if(!entry||typeof entry!=='object'||!entry.name) return;
      var norm=normalizeCatalogueName(entry.name);
      if(objSeen[norm]) return;
      objSeen[norm]=1;
      var syns: string[]=[];
      if(Array.isArray(entry.synonyms)) entry.synonyms.forEach(function(s: any){ syns.push(String(s)); });
      else if(typeof entry.synonyms==='string'&&entry.synonyms) syns.push(entry.synonyms);
      objects.push({name:String(entry.name), kind:normalizeCatalogueKind(entry.kind), synonyms:syns});
    });
    (colList||[]).forEach(function(c: any){
      if(c&&c.table&&c.name) columns.push({table:String(c.table), name:String(c.name),
    ...
    src.split(/\r?\n/).forEach(function(line){
      ...
      var name=toks[0], kind: CatalogueKind='OTHER', syns: string[]=[];
      var ci=1;
      if(toks[ci]&&CATALOGUE_KINDS[toks[ci].toUpperCase()]===1){ kind=normalizeCatalogueKind(toks[ci]); ci++; }
      for(;ci<toks.length;ci++) syns.push(toks[ci]);
...
function catalogueSummary(cat: Catalogue | null | undefined): string {
  if(!cat) return 'No catalogue loaded.';
  var n=cat.objects.length, c=cat.columns.length, conf=Object.keys(cat.conflicts||{}).length;
  var bits=[n+' object'+(n===1?'':'s')];
  if(c) bits.push(c+' column'+(c===1?'':'s'));
  if(conf) bits.push(conf+' conflict'+(conf===1?'':'s'));
  return bits.join(', ');
}
