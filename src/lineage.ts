...
function buildQueryGraph(stmtToks: Token[], header: SqlHeader, opts?: AnalyseOptions): Graph {
  ...
  function link(a: string, b: string, label?: string): void {
    if(a&&b&&a!==b) edges.push({from:a,to:b,label:label||'',style:'solid',
                                kind:'dependency'});
  }
  function descr(r: QueryReferenceInfo): string {
    var bits: string[]=[];
  ...
  function srcNode(name: string): string {
    ...
    if(!srcIds[k]){
      var res: {resolution: CatalogueResolution; resolvedName?: string}=
        opts.catalogue?resolveCatalogue(opts.catalogue, name):{resolution:'external'};
  ...
  var memo={};
  function depthOf(id: string, guard?: number): number {
 ...
 */
function buildObjectQueryGraph(ast: AstNode[], header: SqlHeader, opts?: AnalyseOptions): Graph {
  opts=opts||{};
  ...
  statements.forEach(function(toks){
    ...
    var childOpts: AnalyseOptions={sources:opts.sources, catalogue:opts.catalogue};
    childOpts.finalLabel=summarise(finalToks,64);
    var child=buildQueryGraph(toks,{name:''},childOpts);
    var remap: Record<string, string>={};
