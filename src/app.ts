...
(function(){
  ...
  var currentReport: ReportParseResult | null=null,
      currentReportText: string='',
      currentReportDiagnostics: Diagnostic[]=[];
...
"END;"].join('\n');
  ...
  function applyReport(): void {
    ...
    if(!currentReportText.trim()){
      currentReport=null; currentReportDiagnostics=[];
    } else {
  ...
  function updateReportStatus(): void {
  ...
  function applySnapshot(snap: WorkspaceSnapshot): void {
    ...
    } else { currentReport=null; currentReportDiagnostics=[]; }
    updateReportStatus();
    populateReportDatasets();
  ...
  function mergeConstructCoverage(acc: ConstructCoverage, cc?: ConstructCoverage): void {
    if(!cc) return;
    acc.constructs+=cc.constructs; acc.resolved+=cc.resolved; acc.opaque+=cc.opaque;
    Object.keys(cc.byKind||{}).forEach(function(k){
      ...
      into.detected+=from.detected; into.resolved+=from.resolved; into.opaque+=from.opaque;
    });
  }

  function setConstructCoverage(cc: ConstructCoverage | null): void {
  ...
  function run(): void {
    ...
    try{
      ...
      populateObjects();
      object=activeObject();
      if(object&&sql.value!==object.source) loadObjectSource(object);
    }catch(err){
    ...
    if(scope==='dependencies'){
      ...
      var estateCoverage: ConstructCoverage={constructs:0,resolved:0,opaque:0,byKind:{}};
    ...
    showDiagnostics(result.diagnostics,
      result.mode==='flow'&&result.dialect==='sqlite'&&st.branch+st.loop+st.cat===0
    ...
    setAnalysisHealth(result.confidence,result.coverage,
      result.diagnostics.filter(function(d){return d.severity!=='info';}).length);
    ...
    Array.prototype.forEach.call(elements,function(el){
      ...
      var open=function(e){
        ...
        if(navigable){
          ...
          run();
        } else highlightSource(node.source);
      };
  ...
  window.addEventListener('mousemove',function(e){ if(!dragging) return; panX=e.clientX-sx; panY=e.clientY-sy; apply(); });
  ...
  $('file-input').onchange=function(){
    ...
    Promise.all(files.map(function(file){
      ...
      workspaceFiles=loaded; estate=null; activeObjectId=null;
      run();
  ...
  $('btn-clear').onclick=function(){
    workspaceFiles=null; estate=null; activeObjectId=null;
    sql.value=''; drawGutter(); run(); sql.focus();
  };
  ...
  $('btn-svg').onclick=function(){
    var svg=stage.querySelector('svg');
    if(!svg){ showMsg('Draw a chart first, then it can be saved.'); return; }
  ...
  $('btn-drawio').onclick=function(){
    if(!lastGraph){ showMsg('Draw a chart first, then it can be exported to draw.io.'); return; }
  ...
  $('btn-report-clear').onclick=function(){
    $('report-text').value='';
    currentReport=null; currentReportDiagnostics=[]; currentReportText='';
    updateReportStatus();
    populateReportDatasets();
    run();
  };
  ...
  $('report-dataset-select').onchange=function(){
    ...
    if(!currentReport||isNaN(idx)) return;
    ...
    estate=null; activeObjectId=null;
    drawGutter(); run();
  };
  updateReportStatus();
  populateReportDatasets();
