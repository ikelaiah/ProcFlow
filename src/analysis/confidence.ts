...
function walkConfidenceAst(list: AstNode[] | null | undefined,
    visit: (statement: AstNode) => void): void {
  (list||[]).forEach(function(st: AstNode){
    visit(st);
    if(st.type==='block') walkConfidenceAst(st.body,visit);
    else if(st.type==='if'){
      if(st.then) walkConfidenceAst([st.then],visit);
      if(st.else) walkConfidenceAst([st.else],visit);
    } else if(st.type==='case'){
      st.branches.forEach(function(branch: {cond: TokenList; body: AstNode[]}){
      ...
      walkConfidenceAst(st.body,visit);
      st.handlers.forEach(function(handler: ExceptionHandler){
...
function confidenceSpanOfTokens(toks?: Token[] | null): SourceSpan | null {
  if(!toks||!toks.length) return null;
  return {start:toks[0].pos,end:toks[toks.length-1].end};
}

function confidenceTokensOf(node: AstNode): TokenList | undefined {
  switch(node.type){
    case 'stmt': case 'dynamic': case 'return': case 'throw':
    case 'sqlite_raise': case 'break': case 'continue': case 'label':
    case 'goto': case 'unknown':
      return node.toks;
    default:
      return undefined;
  }
...
function analyseConfidence(ast: AstNode[], diagnostics: Diagnostic[],
  ...
  walkConfidenceAst(ast,function(st){
    ...
    var regionSpan: SourceSpan=span;
