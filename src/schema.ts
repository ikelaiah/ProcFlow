...
interface SchemaCtx {
  ...
  byKey: Record<string, SchemaEntity>;
  diagnostics: Diagnostic[];
  pendingIndexes: SchemaPendingIndex[];
}

interface SchemaPendingIndex {
  tableNorm: string;
  tableName: string;
  columns: string[];
  name?: string;
...
interface SchemaReferenceParse {
...
interface SchemaParsedColumns {
  names: string[];
  end: number;
}

var SCHEMA_IGNORED_CREATE: StringSet = S([
...
function schemaUnquote(part: string): string {
  var s=part||'';
  if(s.length>=2){
    var a=s.charAt(0), b=s.charAt(s.length-1);
    if(a==='['&&b===']') s=s.slice(1,-1).replace(/\]\]/g,']');
    else if(a==='"'&&b==='"') s=s.slice(1,-1).replace(/""/g,'"');
    else if(a==='`'&&b==='`') s=s.slice(1,-1).replace(/``/g,'`');
  }
  return s;
}

function schemaNormName(parts: string[]): string {
  return parts.map(schemaUnquote).join('.').replace(/\s+/g,' ').trim().toUpperCase();
}

function schemaNormColumn(name: string): string {
...
function schemaFingerprint(result: SchemaResult): string {
...
function schemaSpan(start: number, end: number): SourceSpan {
...
function schemaIdent(toks: Token[], i: number): SchemaIdentParts | null {
  if(i>=toks.length||toks[i].type!=='word') return null;
...
function schemaSkipParens(toks: Token[], open: number): number {
  var depth=0, i=open;
  while(i<toks.length){
    if(toks[i].v==='(') depth++;
    else if(toks[i].v===')'){
...
function schemaSkipDefault(toks: Token[], i: number): number {
  var depth=0;
  while(i<toks.length){
    var t=toks[i];
    if(t.v==='('){ depth++; i++; continue; }
    if(t.v===')'){
      if(depth===0) break;
      depth--; i++; continue;
    }
    if(depth===0){
      if(t.v===',') break;
      if(t.type==='word'&&SCHEMA_STOP[t.u]) break;
    }
    i++;
  }
  return i;
...
function schemaParseAction(toks: Token[], i: number): {text: string; end: number} {
  var words: string[]=[];
  while(i<toks.length&&toks[i].type==='word'&&words.length<2){
    var u=toks[i].u;
    if(words.length===1&&/^(CASCADE|NULL|DEFAULT|ACTION)$/.test(u)) words.push(u);
    else if(words.length===0&&/^(CASCADE|RESTRICT|SET|NO)$/.test(u)) words.push(u);
    else break;
    i++;
  }
...
function schemaParseReference(toks: Token[], i: number): SchemaReferenceParse | null {
  if(!toks[i]||toks[i].type!=='word'||toks[i].u!=='REFERENCES') return null;
  var id=schemaIdent(toks,i+1);
  if(!id) return null;
  var target: SchemaReference={name:id.parts.join('.'),
                               columns:[],
                               resolution:'opaque',
  ...
  if(toks[j]&&toks[j].v==='('){
    ...
    if(parsed){ target.columns=parsed.names; j=parsed.end; }
  }
  var onDelete: string|undefined, onUpdate: string|undefined, deferrable: boolean|undefined;
  while(j<toks.length){
    var t=toks[j];
    if(t.type==='word'&&t.u==='ON'&&toks[j+1]&&toks[j+1].type==='word'&&
      ...
      if(toks[j+1].u==='DELETE') onDelete=action.text; else onUpdate=action.text;
      j=action.end;
      continue;
    }
    if(t.type==='word'&&t.u==='MATCH'){ j+=2; continue; }
    if(t.type==='word'&&t.u==='DEFERRABLE'){ deferrable=true; j++; continue; }
    if(t.type==='word'&&t.u==='NOT'&&toks[j+1]&&toks[j+1].u==='DEFERRABLE'){
      deferrable=false; j+=2; continue;
    }
    if(t.type==='word'&&t.u==='INITIALLY'){ j+=2; continue; }
    break;
  }
...
function schemaParseColumns(toks: Token[], i: number): SchemaParsedColumns | null {
  if(!toks[i]||toks[i].v!=='(') return null;
  var names: string[]=[], j=i+1;
  while(j<toks.length){
    var t=toks[j];
    if(t.v===')') return {names:names,end:j+1};
    if(t.v===','){ j++; continue; }
    if(t.type==='word'){
      ...
      j=id.next;
      while(j<toks.length&&toks[j].type==='word'&&(toks[j].u==='ASC'||toks[j].u==='DESC')) j++;
      continue;
    }
    return null;
  }
...
function schemaAddEntity(ctx: SchemaCtx, entity: SchemaEntity): boolean {
  if(ctx.byKey[entity.id]){
    schemaIssue(ctx,'warning','schema_duplicate_entity',
      'Table or view "'+entity.name+'" is defined more than once; the first definition is kept.',
      entity.span);
    return false;
  }
...
function schemaAddColumn(ctx: SchemaCtx, entity: SchemaEntity, col: SchemaColumn): void {
  if(entity.columns.some(function(c){ return schemaNormColumn(c.name)===schemaNormColumn(col.name); })){
    schemaIssue(ctx,'warning','schema_duplicate_column',
      'Column "'+col.name+'" appears more than once in "'+entity.name+'"; the first definition is kept.',
      col.span);
    return;
  }
...
function schemaAddKey(entity: SchemaEntity, key: SchemaKey): void {
  ...
  if(!duplicate) entity.keys.push(key);
}
...
function schemaSplitItems(toks: Token[], open: number): {items: Token[][]; end: number} {
  var items: Token[][]=[], cur: Token[]=[], depth=1, i=open+1;
  while(i<toks.length){
    var t=toks[i];
    if(t.v==='(') depth++;
    else if(t.v===')'){
...
function schemaStatements(toks: TokenList): Token[][] {
  var out: Token[][]=[], cur: Token[]=[], depth=0;
  function flush(): void {
...
function schemaMarkPrimaryKey(entity: SchemaEntity, columns: string[]): void {
...
function schemaMarkUnique(entity: SchemaEntity, columns: string[]): void {
  if(columns.length!==1) return;
  entity.columns.forEach(function(col){
    if(schemaNormColumn(col.name)===schemaNormColumn(columns[0])) col.unique=true;
  });
}

/* Column definition: name, raw type, inline constraints, inline FK. */
function schemaParseColumnItem(ctx: SchemaCtx, entity: SchemaEntity, item: Token[],
                               constraintName?: string): void {
  if(!item.length||item[0].type!=='word') return;
  var nameTok=item[0];
  ...
  while(i<item.length){
    ...
    if(t.v==='('){
      ...
      if(typeStart>=0&&afterParen>i) typeEnd=item[afterParen-1].end;
      i=afterParen;
      continue;
    }
    if(t.v===',') break;
    if(typeStart<0) typeStart=t.pos;
    typeEnd=t.end;
    i++;
  }
  if(typeStart>=0&&typeEnd>typeStart){
    col.type=ctx.text.slice(typeStart,typeEnd).replace(/\s+/g,' ').trim();
    if(/^(BIGSERIAL|SERIAL|SMALLSERIAL)$/i.test(col.type)) col.identity=true;
  }
  while(i<item.length){
    var c=item[i];
    if(c.type==='word'&&c.u==='CONSTRAINT'){ i+=2; continue; }
    if(c.type==='word'&&c.u==='NOT'&&item[i+1]&&item[i+1].u==='NULL'){
      col.nullable=false; i+=2; continue;
    }
    if(c.type==='word'&&c.u==='NULL'){ col.nullable=true; i++; continue; }
    if(c.type==='word'&&c.u==='DEFAULT'){ i=schemaSkipDefault(item,i+1); continue; }
    if(c.type==='word'&&c.u==='PRIMARY'&&item[i+1]&&item[i+1].u==='KEY'){
      col.primaryKey=true; col.nullable=false;
    ...
    if(c.type==='word'&&c.u==='CHECK'){ i=schemaSkipParens(item,i+1); continue; }
    if(c.type==='word'&&c.u==='GENERATED'){
      var g=i+1;
      if(item[g]&&item[g].type==='word'&&(item[g].u==='ALWAYS'||item[g].u==='BY')){
        if(item[g].u==='BY'){
          g++;
          if(item[g]&&item[g].u==='DEFAULT') g++;
          if(item[g]&&item[g].u==='ON'&&item[g+1]&&item[g+1].u==='NULL') g+=2;
        } else g++;
      }
      if(item[g]&&item[g].u==='AS'){
        ...
        if(item[g]&&item[g].u==='IDENTITY'){
          ...
          col.generated=true; g=schemaSkipParens(item,g);
        }
    ...
    if(c.type==='word'&&c.u==='AUTO_INCREMENT'){ col.identity=true; i++; continue; }
    if(c.type==='word'&&c.u==='AUTOINCREMENT'){ col.identity=true; i++; continue; }
    if(c.type==='word'&&c.u==='COLLATE'){ i+=2; continue; }
    if(c.type==='word'&&c.u==='COMMENT'){ i+=2; continue; }
    if(c.type==='word'&&c.u==='AS'&&item[i+1]&&item[i+1].v==='('){
      col.computed=true; i=schemaSkipParens(item,i+1); continue;
    }
    if(c.v==='('){ i=schemaSkipParens(item,i); continue; }
    i++;
  }
...
function schemaParseTableItem(ctx: SchemaCtx, entity: SchemaEntity, item: Token[],
                              constraintName?: string): void {
  if(!item.length) return;
  var i=0, spanStart=item[0].pos;
  if(item[0].type==='word'&&item[0].u==='CONSTRAINT'){
    if(item[1]) constraintName=schemaUnquote(item[1].v);
    i=2;
  }
  var head=item[i];
  ...
  if(u==='PRIMARY'||u==='UNIQUE'){
    var kind: SchemaKeyKind=u==='UNIQUE'?'unique':'pk';
    var j=i+1;
    if(item[j]&&item[j].u==='KEY') j++;
    if(u==='UNIQUE'&&item[j]&&(item[j].u==='INDEX')) j++;
    if(item[j]&&(item[j].u==='CLUSTERED'||item[j].u==='NONCLUSTERED')) j++;
    var cols=schemaParseColumns(item,j);
    if(!cols){
      schemaIssue(ctx,'warning','schema_unparsed_statement',
        'Could not read the column list of a '+u+' constraint on "'+entity.name+'".',
        schemaSpan(spanStart,item[item.length-1].end));
      return;
    }
    var key: SchemaKey={kind:kind,columns:cols.names,name:constraintName,
                        span:schemaSpan(spanStart,item[item.length-1].end)};
    schemaAddKey(entity,key);
    if(kind==='pk') schemaMarkPrimaryKey(entity,cols.names);
    else schemaMarkUnique(entity,cols.names);
    return;
  }
  if(u==='FOREIGN'){
    var k=i+1;
    if(item[k]&&item[k].u==='KEY') k++;
    var fkCols=schemaParseColumns(item,k);
    if(!fkCols||!item[fkCols.end]||item[fkCols.end].u!=='REFERENCES'){
      schemaIssue(ctx,'warning','schema_unparsed_statement',
        'Could not read a FOREIGN KEY constraint on "'+entity.name+'".',
        schemaSpan(spanStart,item[item.length-1].end));
      return;
    }
    ...
    if(!ref){
      schemaIssue(ctx,'warning','schema_unparsed_statement',
        'Could not read the REFERENCES target of a FOREIGN KEY on "'+entity.name+'".',
        schemaSpan(spanStart,item[item.length-1].end));
      return;
    }
...
function schemaParseCreateTable(ctx: SchemaCtx, toks: Token[]): boolean {
  var i=1;
  while(i<toks.length&&toks[i].type==='word'&&
        (toks[i].u==='TEMP'||toks[i].u==='TEMPORARY'||toks[i].u==='GLOBAL'||
         toks[i].u==='LOCAL'||toks[i].u==='UNLOGGED'||toks[i].u==='VIRTUAL')) i++;
  if(!toks[i]||toks[i].u!=='TABLE') return false;
  i++;
  if(toks[i]&&toks[i].u==='IF'&&toks[i+1]&&toks[i+1].u==='NOT'&&toks[i+2]&&toks[i+2].u==='EXISTS') i+=3;
  var id=schemaIdent(toks,i);
  if(!id){
    schemaIssue(ctx,'warning','schema_unparsed_statement',
      'Could not read the table name in a CREATE TABLE statement.',
      schemaSpan(toks[0].pos,toks[toks.length-1].end));
    return true;
  }
  ...
  if(toks[id.next]&&toks[id.next].v==='('){
    ...
    if(split.end>0&&toks[split.end-1]) entity.span=schemaSpan(toks[0].pos,toks[split.end-1].end);
  } else {
    schemaIssue(ctx,'info','schema_columns_unknown',
      'Columns of "'+entity.name+'" are not declared in this DDL (CTAS or LIKE); the entity is shown without attributes.',
...
function schemaParseCreateView(ctx: SchemaCtx, toks: Token[]): boolean {
  ...
  while(i<toks.length&&toks[i].type==='word'&&
        (toks[i].u==='OR'||toks[i].u==='REPLACE'||toks[i].u==='MATERIALIZED'||
         toks[i].u==='TEMP'||toks[i].u==='TEMPORARY')) i++;
  if(!toks[i]||toks[i].u!=='VIEW') return false;
  i++;
  if(toks[i]&&toks[i].u==='IF'&&toks[i+1]&&toks[i+1].u==='NOT'&&toks[i+2]&&toks[i+2].u==='EXISTS') i+=3;
  var id=schemaIdent(toks,i);
  if(!id){
    schemaIssue(ctx,'warning','schema_unparsed_statement',
      'Could not read the view name in a CREATE VIEW statement.',
      schemaSpan(toks[0].pos,toks[toks.length-1].end));
    return true;
  }
  var entity: SchemaEntity={
...
function schemaParseAlterTable(ctx: SchemaCtx, toks: Token[]): boolean {
  if(!toks[1]||toks[1].u!=='TABLE') return false;
  ...
  while(i<toks.length){
    if(toks[i].u!=='ADD'){
      ...
      while(i<toks.length&&!(toks[i].v===','&&true)) i++;
      if(toks[i]&&toks[i].v===','){ i++; continue; }
      break;
    }
    i++;
    if(toks[i]&&toks[i].u==='COLUMN') i++;
    /* Collect one ADD item up to the next depth-0 comma. */
    var item: Token[]=[], depth=0;
    while(i<toks.length){
      var t=toks[i];
      if(t.v==='(') depth++;
      else if(t.v===')') depth=Math.max(0,depth-1);
    ...
    if(!entity){
      schemaIssue(ctx,'warning','schema_alter_unknown_table',
        'ALTER TABLE targets "'+id.parts.join('.')+'", which is not defined in this DDL; the change is not applied.',
        schemaSpan(item[0].pos,item[item.length-1].end));
      if(toks[i]&&toks[i].v===',') i++;
      continue;
    }
    var head=item[0];
...
function schemaParseStatement(ctx: SchemaCtx, toks: Token[]): void {
  var head=toks[0];
  if(!head||head.type!=='word'){
    schemaIssue(ctx,'info','schema_ignored_statement',
      'Statement is not a schema definition and was ignored.',
      schemaSpan(head?head.pos:0,toks[toks.length-1].end));
    return;
  }
  if(head.u==='CREATE'){
    var i=1;
    if(toks[i]&&toks[i].u==='OR') i++;
    if(toks[i]&&toks[i].u==='REPLACE') i++;
    if(toks[i]&&(toks[i].u==='INDEX'||toks[i].u==='UNIQUE')){
      if(schemaParseCreateIndex(ctx,toks)) return;
    }
    var kind=toks[i]?toks[i].u:'';
    if(kind==='TABLE'){ schemaParseCreateTable(ctx,toks); return; }
    if(kind==='VIEW'){ schemaParseCreateView(ctx,toks); return; }
    if(SCHEMA_IGNORED_CREATE[kind]) return;
    if(kind) schemaIssue(ctx,'info','schema_ignored_statement',
      'CREATE '+kind+' is not part of the ERD schema model and was ignored.',
      schemaSpan(head.pos,toks[toks.length-1].end));
    return;
  }
  if(head.u==='ALTER'){
    if(schemaParseAlterTable(ctx,toks)) return;
    return;
  }
...
function schemaApplyIndexes(ctx: SchemaCtx): void {
  ctx.pendingIndexes.forEach(function(index){
    var entity=ctx.byKey[index.tableNorm];
    if(!entity) return; /* defined nowhere; not an ERD claim */
    if(entity.unresolved) return;
...
function schemaForeignKeyIsUnique(entity: SchemaEntity, fkColumns: string[]): boolean {
  var fk=fkColumns.map(schemaNormColumn);
  if(!fk.length) return false;
  return entity.keys.some(function(key){
    if(key.kind==='fk'||!key.columns.length) return false;
    var keyCols=key.columns.map(schemaNormColumn);
    return keyCols.every(function(c){ return fk.indexOf(c)>=0; });
  });
}

function schemaEnsureExternal(ctx: SchemaCtx, ref: SchemaReference, span: SourceSpan): SchemaEntity {
  var id='external:'+ref.norm;
  if(ctx.byKey[id]) return ctx.byKey[id];
  var entity: SchemaEntity={
    id:id,
    name:ref.name,
    schema:null,
...
function schemaResolve(ctx: SchemaCtx): SchemaRelationship[] {
  var relationships: SchemaRelationship[]=[], relIndex=0;
  ctx.entities.slice().forEach(function(entity){
    if(entity.unresolved) return;
    entity.keys.forEach(function(key){
      if(key.kind!=='fk'||!key.referenced) return;
      var ref=key.referenced;
      var target=ctx.byKey[ref.norm];
      var resolution: SchemaLinkResolution='opaque';
      if(target){
        resolution='exact';
      } else {
        ...
        if(candidates.length===1){
          ...
          schemaIssue(ctx,'warning','schema_unresolved_reference',
            'Foreign key on "'+entity.name+'" references "'+ref.name+'"; resolved by name only to "'+
            target.name+'" — verify schema qualification.',
            key.span);
        } else if(candidates.length>1){
          schemaIssue(ctx,'warning','schema_ambiguous_reference',
            'Foreign key on "'+entity.name+'" references "'+ref.name+'", which matches '+candidates.length+
            ' tables; the relationship is not drawn.',
            key.span);
          return;
        }
      }
      if(!target){
        ...
        schemaIssue(ctx,'warning','schema_unresolved_reference',
          'Foreign key on "'+entity.name+'" references "'+ref.name+'", which is not defined in this DDL; shown as an unresolved entity.',
          key.span);
      }
...
function schemaStats(result: SchemaResult): SchemaStats {
  var tables=0, views=0, columns=0, unresolved=0;
  result.entities.forEach(function(entity){
    if(entity.unresolved) unresolved++;
    else if(entity.kind==='view') views++;
    else tables++;
...
function parseSchema(text: string): SchemaResult {
  var source=String(text==null?'':text);
  var toks=tokenize(source);
  var ctx: SchemaCtx={text:source,entities:[],byKey:{},
  ...
  var result: SchemaResult={text:source,entities:ctx.entities,
