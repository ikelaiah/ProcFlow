...
function safeMermaidMetadata(value) {
...
function provenanceComment(graph) {
    ...
    (graph.nodes || []).forEach(function (n) {
        ...
        if (n.provenance)
            bits.push('provenance=' + safeMermaidMetadata(n.provenance));
        ...
        if (n.objectId)
            bits.push('object=' + safeMermaidMetadata(n.objectId));
        if (n.resolution)
            bits.push('resolution=' + safeMermaidMetadata(n.resolution));
        if (n.resolvedName)
            bits.push('resolved=' + safeMermaidMetadata(n.resolvedName));
        if (n.reason)
            bits.push('reason=' + safeMermaidMetadata(n.reason));
    ...
    components.forEach(function (comp) {
        ...
        function dfs(root) {
            if (state[root] === 2)
                return;
            var frames = [];
            function push(id, guard) {
                if (guard > dfsLimit || state[id] === 2)
                    return;
                state[id] = 1;
            ...
            while (frames.length) {
                ...
                if (state[to] === 1)
                    localBack.push({ from: frame.id, to: to });
                else if (state[to] !== 2)
                    push(to, frame.guard + 1);
            }
        }
