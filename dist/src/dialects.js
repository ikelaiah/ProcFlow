...
function detectDialect(sql) {
    ...
    if (/\$\$|\$[a-z_]+\$/i.test(s))
        sc.plpgsql += 4;
    if (/LANGUAGE\s+'?plpgsql/i.test(s))
        sc.plpgsql += 7;
    if (/\bELSIF\b/i.test(s))
        sc.plpgsql += 3;
    if (/\bRAISE\s+(NOTICE|EXCEPTION|WARNING|INFO|DEBUG|LOG|USING)\b/i.test(s))
        sc.plpgsql += 4;
    if (/\bPERFORM\b|\bRETURN\s+(NEXT|QUERY)\b/i.test(s))
        sc.plpgsql += 4;
    if (/\bEXCEPTION\s+WHEN\b/i.test(s))
        sc.plpgsql += 4;
    if (/::\s*\w/.test(s))
        sc.plpgsql += 1;
    if (/\bELSEIF\b/i.test(s))
        sc.db2 += 4;
    if (/\bEND\s+(WHILE|REPEAT|FOR)\b/i.test(s))
        sc.db2 += 2;
    if (/\bDECLARE\s+(CONTINUE|EXIT|UNDO)\s+HANDLER\b/i.test(s))
        sc.db2 += 6;
    if (/\bSIGNAL\s+SQLSTATE\b|\bRESIGNAL\b|\bITERATE\b/i.test(s))
        sc.db2 += 4;
    if (/\bLANGUAGE\s+SQL\b/i.test(s) && !/plpgsql/i.test(s))
        sc.db2 += 3;
    if (/\bSPECIFIC\b|\bDYNAMIC\s+RESULT\s+SETS\b|\bSYSIBM\b|\bSYSCAT\b|\bVALUES\s+INTO\b/i.test(s))
        sc.db2 += 2;
    if (/@\w/.test(s))
        sc.tsql += 3;
    if (/\bBEGIN\s+TRY\b|\bBEGIN\s+CATCH\b/i.test(s))
        sc.tsql += 7;
    if (/\bSET\s+NOCOUNT\b|\bRAISERROR\b|\bsp_\w|\bdbo\.|\bEXEC(UTE)?\s+\w/i.test(s))
        sc.tsql += 3;
    if (/\[[A-Za-z_#@]/.test(s))
        sc.tsql += 2;
    if (/^\s*GO\s*;?\s*$/im.test(s))
        sc.tsql += 3;
    if (/\bAUTOINCREMENT\b|\bPRAGMA\b|\bsqlite_|\bINSERT\s+OR\s+(REPLACE|IGNORE)\b/i.test(s))
        sc.sqlite += 5;
    if (/\bRAISE\s*\(\s*(IGNORE|FAIL|ABORT|ROLLBACK)\b/i.test(s))
        sc.sqlite += 7;
    if (/\bCREATE\s+TRIGGER\b/i.test(s) && /\bFOR\s+EACH\s+ROW\b/i.test(s) &&
        !/\bDECLARE\b|\bLANGUAGE\b/i.test(s))
        sc.sqlite += 2;
...
function readTokens(p, mode) {
    ...
    while (p.i < p.t.length) {
        ...
        if (t.v === ';' && depth === 0 && caseDepth === 0) {
            if (mode !== 'cond')
                p.i++;
            break;
        }
        if (t.type === 'word' && depth === 0 && !merge) {
            if (t.u === 'CASE' && out.length)
                caseDepth++;
            else if (t.u === 'END') {
                if (caseDepth > 0) {
                    caseDepth--;
                ...
                if (mode === 'head') {
                    ...
                    if (t.u === 'WHEN' || t.u === 'THEN' || t.u === 'ELSE')
                        break;
                }
                else if (mode === 'cond') {
                    if (OPENERS[t.u] || p.hard[t.u] || SOFT_M[t.u])
                        break;
                }
                else {
                    if (p.hard[t.u])
                        break;
                    if (SOFT_M[t.u] && newStatementHere(t, prev, startWord))
                        break;
                }
            }
        }
...
function eatEnd(p, kind) {
    ...
    if (kind && at(p, kind))
        ...
        ['IF', 'WHILE', 'LOOP', 'FOR', 'REPEAT', 'CASE'].indexOf(peek(p).u) >= 0)
        p.i++;
...
function parseStatement(p) {
    ...
    if (t.type === 'word' && u === 'BEGIN') {
        ...
        eatEnd(p, null);
        if (hs.length)
            return { type: 'try', body: body, handlers: hs };
    ...
    if (t.type === 'word' && u === 'IF') {
        ...
        if (eat(p, 'THEN')) { /* DB2 / PL/pgSQL form */
            ...
            if (eat(p, 'ELSE'))
                tail.else = { type: 'block', body: parseBlock(p, []) };
            eatEnd(p, 'IF');
            return node;
        }
    ...
    if (t.type === 'word' && (u === 'EXEC' || u === 'EXECUTE')) {
        ...
        if (dynamic)
            return { type: 'dynamic', toks: readTokens(p, 'stmt') };
    }
    ...
    if (t.type === 'word' && (u === 'BREAK' || u === 'LEAVE' || u === 'EXIT' || u === 'CONTINUE' || u === 'ITERATE')) {
        ...
        if (nt && nt.type === 'word' && nt.u !== 'WHEN' && nt.v !== ';') {
            target = nt.v;
            p.i++;
        }
        if (eat(p, 'WHEN'))
...
function findBody(toks, dialect, src) {
    ...
    while (i < toks.length && ['SET', 'USE', 'GO', 'PRAGMA'].indexOf(toks[i].u) >= 0) {
        while (i < toks.length && toks[i].v !== ';' && toks[i].u !== 'GO')
            i++;
        if (i < toks.length)
            i++;
    }
    ...
    var j = i + 1;
    while (toks[j] && ['OR', 'ALTER', 'REPLACE', 'TEMP', 'TEMPORARY', 'MATERIALIZED', 'UNIQUE', 'CLUSTERED'].indexOf(toks[j].u) >= 0)
        j++;
    ...
    for (k = afterName; k < toks.length && k < afterName + 120; k++) {
        ...
        if (pt.v === ')') {
            depth--;
            if (depth <= 0 && ps.length)
                break;
            continue;
        }
        if (pt.u === 'AS' || pt.u === 'BEGIN' || pt.u === 'RETURNS') {
            if (depth <= 0)
                break;
        }
        if (depth === 1 || pt.v.charAt(0) === '@') {
            ...
            if (named && ps.length < 10 && ['IN', 'OUT', 'INOUT', 'VARIADIC'].indexOf(pt.u) < 0)
                ps.push(pt.v);
        }
    }
    params = ps.join(', ');
    ...
    if (dialect === 'sqlite' || kind === 'TRIGGER') {
        ...
        for (k = afterName; k < toks.length; k++) {
            if (toks[k].v === '(')
                ...
                d2--;
            else if (d2 === 0 && toks[k].u === 'WHEN' && w < 0)
                w = k;
            else if (d2 === 0 && toks[k].u === 'BEGIN') {
                b = k;
                break;
            }
        }
    ...
    for (k = afterName; k < toks.length; k++) {
        ...
        if (tk.v === '(')
            ...
            if (!(toks[k - 1] && toks[k - 1].u === 'EXECUTE') &&
                ...
                !(nn && nn.type === 'str'))
                return { name: name, params: params, kind: kind, index: k + 1, gate: null };
        }
