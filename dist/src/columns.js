...
var COL_CLAUSE_START = S(['FROM', 'WHERE', 'GROUP', 'HAVING', 'ORDER', 'LIMIT',
    'OFFSET', 'FETCH', 'UNION', 'EXCEPT', 'INTERSECT',
    'INTO', 'FOR', 'OPTION', 'RETURNING']);
var COL_ARM_SEP = S(['UNION', 'EXCEPT', 'INTERSECT']);
var COL_JOIN_PREFIX = S(['JOIN', 'APPLY', 'CROSS', 'INNER', 'LEFT', 'RIGHT',
    'FULL', 'OUTER', 'LATERAL']);
/* Words inside an expression that are never column references. */
var COL_REF_SKIP = S(['SELECT', 'WHERE', 'GROUP', 'HAVING', 'ORDER', 'LIMIT',
...
function catalogueColumnsFor(cat, tableName) {
    if (!cat || !cat.columns || !cat.columns.length || !tableName)
        return [];
    ...
    cat.columns.forEach(function (c) {
        if (normalizeCatalogueName(c.table) !== norm)
            return;
...
function colTokensInSpan(toks, span) {
    if (!span)
        return 0;
    var n = 0;
    for (var i = 0; i < toks.length; i++) {
        var t = toks[i];
        if (t.pos < span.end && t.end > span.start)
            n++;
    }
    return n;
}
function colSplitArms(toks) {
    var arms = [], cur = [], d = 0;
    for (var i = 0; i < toks.length; i++) {
        var t = toks[i];
        if (t.v === '(') {
        ...
        while (j < toks.length && guard++ < 40) {
            ...
            if (u === 'TOP') {
                ...
                if (toks[j] && toks[j].v === '(') {
                    ...
                    if (toks[j] && toks[j].type === 'num')
                        j++;
                    if (toks[j] && toks[j].u === 'PERCENT')
                        j++;
                }
                continue;
            }
    ...
    if (fromStart >= 0) {
        ...
        for (var m = fromStart; m < toks.length; m++) {
            ...
            if (t3.v === '(')
                ...
                d3--;
            if (d3 === 0 && m > fromStart && COL_CLAUSE_START[t3.u] === 1) {
                fromEnd = m;
                break;
            }
        }
...
function colParseSources(fromToks, ctx) {
    var sources = [], predicates = [];
    var i = 0, guard = 0;
    function predEnd(after) {
        var d = 0;
        for (var k = after; k < fromToks.length; k++) {
            ...
            if (d === 0 && k > after && (COL_CLAUSE_START[t.u] === 1 ||
                (t.type === 'word' && COL_JOIN_PREFIX[t.u] === 1 && t.u !== 'CASCADE') || t.v === ','))
                return k;
        }
    ...
    while (i < fromToks.length && guard++ < 400) {
        var t = fromToks[i];
        if (t.type === 'word' && COL_JOIN_PREFIX[t.u] === 1) {
            i++;
            continue;
        }
        if (t.v === ',') {
        ...
        if (t.v === '(') {
            ...
            while (j < fromToks.length && d > 0) {
                if (fromToks[j].v === '(')
                    d++;
                else if (fromToks[j].v === ')')
                    d--;
                j++;
            }
            var inner = fromToks.slice(innerStart, j - 1);
            var k2 = j;
            while (k2 < fromToks.length && fromToks[k2].u === 'AS')
                k2++;
            var alias = null;
            var colAliasTok = null;
            if (k2 < fromToks.length && fromToks[k2].type === 'word' &&
                ...
                if (fromToks[k2] && fromToks[k2].v === '(') {
                    ...
                    while (g < fromToks.length && dd > 0) {
                        if (fromToks[g].v === '(')
                            dd++;
                        else if (fromToks[g].v === ')')
                            dd--;
                        if (dd >= 1)
                            innerAlias.push(fromToks[g]);
                        g++;
                    }
            ...
            if (headT === 'SELECT') {
                ...
                if (innerRes)
                    derivedCols = innerRes.outputs.map(function (o) { return o.name; });
            }
            ...
            if (colAliasTok && colAliasTok.length) {
                ...
                if (headT !== 'SELECT' && columns.length)
                    kind = 'derived';
            }
            ...
            var name = q.join('.');
            var k3 = kq + 1;
            while (k3 < fromToks.length && ['FULLTEXT', 'WINDOW'].indexOf(fromToks[k3].u) >= 0)
                k3++;
            var alias2 = null;
            var colAlias2 = null;
            if (fromToks[k3] && fromToks[k3].u === 'AS')
                k3++;
            if (fromToks[k3] && fromToks[k3].type === 'word' &&
                ...
                alias2 = fromToks[k3].v;
                k3++;
                if (fromToks[k3] && fromToks[k3].v === '(') {
                    var dd2 = 1, g2 = k3 + 1, innerAlias2 = [];
                    while (g2 < fromToks.length && dd2 > 0) {
                        if (fromToks[g2].v === '(')
                            dd2++;
                        else if (fromToks[g2].v === ')')
                            dd2--;
                        if (dd2 >= 1)
                            innerAlias2.push(fromToks[g2]);
                        g2++;
                    }
            ...
            if (colAlias2 && colAlias2.length)
                cols2 = colNamesOf(colAlias2);
...
function colBind(qualifier, col, span, sources, diagnostics) {
...
function colScanRefs(toks, sources, diagnostics) {
    var refs = [], bound = [];
    var opaqueSpans = [];
    function addRef(qualifier, name, span) {
    ...
    while (i < toks.length) {
        ...
        if (t.v === '(') {
            if (toks[i + 1] && toks[i + 1].u === 'SELECT') {
                ...
                var subSpan = { start: t.pos, end: toks[e - 1].end };
...
function colParseItems(projToks) {
    var items = [];
    var cur = [], d = 0;
    ...
    items.forEach(function (item) {
        ...
        if (!toks.length)
            return;
        var alias = null, expr = toks.slice();
...
function colOutputName(expr, refs) {
    if (!expr.length)
        return '';
    if (expr.length === 1)
        return expr[0].v;
...
function colSourceByKey(sources, key) {
    var up = key.toUpperCase();
    for (var i = 0; i < sources.length; i++)
        if (sources[i].key.toUpperCase() === up)
            return sources[i];
    return null;
}
/* Analyse one SELECT arm. */
function colAnalyseArm(toks, ctx, first) {
    if (!toks.length || toks[0].u !== 'SELECT')
        return null;
    ...
    items.forEach(function (item) {
        ...
        for (var w = 0; w < item.expr.length; w++) {
            ...
            var span = { start: wt.pos, end: wt.end };
            var wc = { source: exp ? exp.key : null, span: span, expanded: false, columns: [] };
        ...
        references = references.concat(scan.refs);
        var resolution = 'exact';
        var bindings = [];
        var reason = undefined;
        scan.bound.forEach(function (b) {
            if (b.resolution === 'opaque') {
                resolution = 'opaque';
                reason = 'unsupported or unresolvable expression';
            }
            else if (b.resolution === 'ambiguous' && resolution === 'exact') {
        ...
        if (scan.opaqueSpans.length) {
            resolution = 'opaque';
            reason = 'subquery expression is opaque';
        }
        scan.opaqueSpans.forEach(function (sp) { opaqueCount += Math.max(1, colTokensInSpan(toks, sp)); });
...
function colAnalyseToks(toks, ctx) {
    if (!toks.length)
        return null;
    if (ctx.depth > 8)
        return null;
    ...
    split.ctes.forEach(function (cte) {
        var child = { cteColumns: cteMap, catalogue: ctx.catalogue,
        ...
        if (inner) {
            ...
            inner.sources.forEach(function (s) { if (!colSourceByKey(sourcesAll, s.key))
                sourcesAll.push(s); });
        }
    });
    var finalToks = toks.slice(split.finalStart);
    if (!finalToks.length || finalToks[0].u !== 'SELECT')
        return null;
    ...
    for (var a = 0; a < arms.length; a++) {
        ...
        arm.sources.forEach(function (s) { if (!colSourceByKey(sourcesAll, s.key))
            sourcesAll.push(s); });
        if (!firstDone) {
            outputs = arm.outputs;
            firstDone = true;
...
function colTrimStatement(toks) {
    var end = toks.length;
    while (end > 0 && toks[end - 1].v === ';')
        end--;
    var d = 0;
    for (var i = 0; i < end; i++) {
        if (toks[i].v === '(')
            ...
            d--;
        else if (d <= 0 && toks[i].type === 'word' && toks[i].u === 'GO') {
            end = i;
            break;
        }
    }
...
function analyseColumns(toks, opts) {
    opts = opts || {};
    var trim = colTrimStatement(toks || []);
    if (trim.length !== (toks || []).length)
        toks = trim;
    var ctx = { cteColumns: {}, catalogue: opts.catalogue || null,
