...
(function () {
    ...
    function applySnapshot(snap) {
        ...
        estate = null;
        activeObjectId = snap.activeObjectId;
    ...
    function run() {
        ...
        if (scope === 'report') {
            ...
            var reportGraph = buildReportGraph(currentReport, { catalogue: currentCatalogue || undefined });
            ...
            lastCode = reportCode;
            lastGraph = filteredReport;
            lastResult = null;
            lastTitle = 'procflow-report';
    ...
    $('rf-cols').onchange = run;
    $('rf-embedded').onchange = run;
    $('rf-shared').onchange = run;
    $('rf-unresolved').onchange = run;
    $('rf-external').onchange = run;
    $('rf-focus').addEventListener('input', schedule);
    $('btn-report-filter-reset').onclick = function () {
        $('rf-cols').checked = true;
        $('rf-embedded').checked = true;
        $('rf-shared').checked = true;
        $('rf-unresolved').checked = true;
        $('rf-external').checked = true;
        $('rf-focus').value = '';
        run();
    };
    ...
    $('btn-ws-restore').onclick = function () {
        ...
        if (!snap) {
            flash($('btn-ws-restore'), 'None saved');
            return;
        }
        applySnapshot(snap);
    };
    $('btn-ws-export').onclick = function () {
        var snap = readWorkspace();
        if (!snap) {
            flash($('btn-ws-export'), 'Nothing saved');
            return;
        }
