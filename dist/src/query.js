...
function queryOtherEnd(edge, nodeId) {
...
function queryDijkstra(graph, sourceId) {
    ...
    while (true) {
        ...
        graph.order.forEach(function (id) {
            if (done[id] || dist[id] === undefined)
                return;
            if (node === null || dist[id] < dist[node] ||
                (dist[id] === dist[node] && id < node))
                node = id;
        });
        if (node === null)
            break;
...
function queryGraphComponents(graph, ids) {
    var wanted = {};
    ids.forEach(function (id) { if (graph.nodes[id])
        wanted[id] = 1; });
    var seen = {}, out = [];
    ids.forEach(function (id) {
        if (!graph.nodes[id] || seen[id])
            return;
        var queue = [id], component = [];
        seen[id] = 1;
        while (queue.length) {
            ...
            graph.nodes[node].edges.forEach(function (edge) {
                var other = queryOtherEnd(edge, node);
                if (other === null || !wanted[other] || seen[other])
                    return;
                seen[other] = 1;
                queue.push(other);
            });
...
function queryManualEdges(graph, manual) {
    var warnings = [];
    (manual || []).forEach(function (entry) {
        if (entry.leftId === entry.rightId)
            return; /* self joins are handled in the plan */
...
/* ===== join plan ===== */
function queryDefaultJoinType(edge, reverse, policy) {
    if (edge.manualId)
        return 'inner';
    if (policy === 'strict')
        return 'inner';
...
function queryJoinExplanation(graph, join, attachToId, entityId) {
...
function queryPathSummary(graph, path) {
...
function queryBuildPlan(input) {
    ...
    selections.forEach(function (sel) {
        if (selectedIds.indexOf(sel.entityId) < 0)
            selectedIds.push(sel.entityId);
    });
    var selected = {};
    selectedIds.forEach(function (id) { selected[id] = 1; });
    var policy = input.policy === 'strict' ? 'strict' : 'preserve';
    var fromId = selections[0].entityId;
    plan.fromId = fromId;
    var used = {};
    used[fromId] = 1;
    var usedOrder = [fromId];
    (input.cross || []).forEach(function (id) {
        if (!graph.nodes[id] || used[id])
            return;
        if (excluded[id])
            return;
        ...
        used[id] = 1;
        usedOrder.push(id);
        var join = { id: 'cross:' + id, kind: 'cross', attachToId: fromId,
        ...
        if (join.bridge && plan.bridges.indexOf(id) < 0)
            plan.bridges.push(id);
    });
    ...
    while (remaining.length && guard++ < QUERY_MAX_STEPS) {
        ...
        for (var stepIndex = path.steps.length - 1; stepIndex >= 0; stepIndex--) {
            ...
            if (join.bridge && plan.bridges.indexOf(entityId) < 0)
                plan.bridges.push(entityId);
        }
    ...
    (input.manual || []).forEach(function (entry) {
        ...
        if (!queryNodeHasColumn(node, entry.leftColumn) ||
            !queryNodeHasColumn(node, entry.rightColumn))
            return;
        ...
        var join = { id: 'manual:' + entry.id, kind: 'manual',
...
/* ===== SQL emission ===== */
function queryQuoteIdent(name, dialect) {
    var s = String(name == null ? '' : name);
    if (dialect === 'tsql')
        return '[' + s.replace(/\]/g, ']]') + ']';
    return '"' + s.replace(/"/g, '""') + '"';
}
function queryQuoteTable(name, dialect) {
    return String(name || '').split('.').map(function (part) {
        return queryQuoteIdent(part, dialect);
    }).join('.');
}
function queryAliasBase(name) {
...
function queryAliasFor(name, taken) {
...
function queryColumnAlias(tableAlias, column) {
    return tableAlias + '_' + String(column || '').replace(/[^A-Za-z0-9_]/g, '').toLowerCase();
}
function querySafeComment(text) {
...
function queryTokenizeSQL(text) {
    var sql = String(text == null ? '' : text);
    var tokens = [], i = 0;
    function push(kind, start, end) {
        if (end > start)
            tokens.push({ text: sql.slice(start, end), kind: kind });
    }
    while (i < sql.length) {
        var ch = sql.charAt(i);
        ...
        if (ch >= '0' && ch <= '9') {
            var digits = i;
            while (digits < sql.length && /[0-9.]/.test(sql.charAt(digits)))
                digits++;
        ...
        if (/[A-Za-z_]/.test(ch)) {
            var wordEnd = i;
            while (wordEnd < sql.length && /[A-Za-z0-9_$]/.test(sql.charAt(wordEnd)))
                wordEnd++;
        ...
        if (/\s/.test(ch)) {
            var spaceEnd = i;
            while (spaceEnd < sql.length && /\s/.test(sql.charAt(spaceEnd)))
                spaceEnd++;
...
function queryJoinCondition(join, dialect, aliasOf) {
    if (join.predicate)
...
var QUERY_AGGREGATE_FNS = ['count', 'count-distinct', 'sum', 'avg', 'min', 'max'];
function queryAggregateLabel(fn) {
    return fn === 'count' ? 'COUNT' : fn === 'count-distinct' ? 'COUNT DISTINCT' :
        fn === 'sum' ? 'SUM' : fn === 'avg' ? 'AVG' : fn === 'min' ? 'MIN' : 'MAX';
}
function queryAggregateExpr(fn, expression) {
    return fn === 'count-distinct'
        ? 'COUNT(DISTINCT ' + expression + ')'
        : queryAggregateLabel(fn) + '(' + expression + ')';
}
function queryAggregateAlias(fn, tableAlias, column) {
    var prefix = fn === 'count-distinct' ? 'count_distinct' : fn;
    return prefix + '_' + queryColumnAlias(tableAlias, column);
}
function queryPlanSQL(plan, options) {
    if (!plan || !plan.fromId)
        return '';
    var dialect = options && options.dialect ? options.dialect : 'tsql';
    ...
    function selectionAlias(entityId, column) {
    ...
    function selectedKey(entityId, column) {
    ...
    Object.keys((options && options.aggregates) || {}).forEach(function (rawKey) {
        ...
        var fn = options.aggregates[rawKey];
        if (selected[key] && QUERY_AGGREGATE_FNS.indexOf(fn) >= 0)
            aggregates[key] = fn;
    });
    ...
    ((options && options.orderBy) || []).forEach(function (sort) {
        if (!sort || !sort.entityId || !sort.column)
            return;
        var key = selectedKey(sort.entityId, sort.column);
        if (!selected[key] || seenSort[key])
            return;
        seenSort[key] = 1;
    ...
    if (sorts.length) {
        var sortParts = sorts.map(function (sort) {
            ...
            if (fn)
                expression = queryAggregateExpr(fn, expression);
    ...
    if (rowLimit) {
        if (dialect === 'postgres' || dialect === 'sqlite')
            ...
            lines.push('FETCH FIRST ' + rowLimit + ' ROWS ONLY');
    }
    ...
    plan.selections.forEach(function (sel) {
        if (plan.usedIds.indexOf(sel.entityId) >= 0)
            return;
    ...
    if (hasAggregates) {
        var headerGroups = [], headerAggregates = [];
        visible.forEach(function (sel) {
            var key = selectedKey(sel.entityId, sel.column);
            var fn = aggregates[key];
            var name = queryNameOf(graph, sel.entityId) + '.' + sel.column;
            if (fn)
                headerAggregates.push(queryAggregateLabel(fn) + '(' + name + ')');
            else
                headerGroups.push(name);
        });
    ...
    if (rowLimit)
        header.push('Row cap: first ' + rowLimit + ' rows only.');
    if (sorts.length) {
    ...
    plan.usedIds.forEach(function (id) {
        ...
        var note = '';
        if (join && join.kind === 'manual')
            note = ', taught join — not declared';
        if (join && join.kind === 'cross')
            note = ', CROSS JOIN — approve the row count';
