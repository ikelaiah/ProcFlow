"use strict";
function qbViewTag(text, className) {
    var tagEl = document.createElement('span');
    tagEl.className = 'qb-tag' + (className ? ' ' + className : '');
    tagEl.textContent = text;
    return tagEl;
}
function qbViewShowButton(entityId) {
...
function qbViewTeachBanner(graph, teachFirst) {
...
function qbViewJoinConditionText(graph, join) {
    if (join.kind === 'cross')
...
function qbViewAmbiguityCard(graph, ambiguity) {
...
function qbViewJoinCard(graph, join, picks) {
    var card = document.createElement('div');
    card.className = 'qb-join';
    ...
    head.appendChild(title);
    if (join.bridge)
        head.appendChild(qbViewTag('bridge', 'qb-tag-bridge'));
    if (join.kind === 'manual') {
    ...
    card.appendChild(note);
    if (join.selfJoin)
        card.appendChild(qbViewCopyEditor(join, picks));
    return card;
}
/* Self joins list the table's picked columns so the user can route each one to
   the second alias. */
function qbViewCopyEditor(join, picks) {
    var box = document.createElement('div');
...
function qbViewColumnOptions(graph) {
...
function qbViewProblemCard(graph, problem, draft, teaching) {
...
function qbViewLearn(education) {
    if (!education.length)
        return null;
...
//# sourceMappingURL=erd-query-view.js.map