...
function queryCloneJoinTypes(value) {
    ...
    Object.keys(value).forEach(function (key) {
        var entry = value[key];
        if (entry === 'inner' || entry === 'left' || entry === 'cross')
            out[key] = entry;
    });
    return out;
}
function queryClonePathChoices(value) {
    var out = {};
    if (!value || typeof value !== 'object')
        return out;
    Object.keys(value).forEach(function (key) {
...
function querySavedSorts(value) {
    var sorts = [];
    (value || []).forEach(function (sort) {
        if (!sort || typeof sort.entityId !== 'string' || typeof sort.column !== 'string')
            return;
...
function queryStateBuild(result, input) {
...
function queryStateToJSON(state) {
    return JSON.stringify(state, null, 1);
}
function queryStateFromJSON(text) {
    var diagnostics = [];
    ...
    if (parsed.selections === undefined) {
        ...
        parsed.selections.forEach(function (entry) {
    ...
    if (parsed.manual === undefined) {
        ...
        parsed.manual.forEach(function (entry) {
            if (entry && typeof entry.id === 'string' && typeof entry.leftId === 'string' &&
                ...
                if (Array.isArray(entry.selfColumns)) {
                    var self = entry.selfColumns.filter(function (name) {
                        return typeof name === 'string' && !!name;
                    });
                    if (self.length)
                        join.selfColumns = self;
                }
                manual.push(join);
            }
            else
...
function queryStatePrune(state, graph) {
    var dropped = [];
    function nodeOf(id) {
...
function queryFileBaseName(state) {
