...
function tokenize(sql) {
    ...
    while (i < n) {
        ...
        if (c === '/' && sql[i + 1] === '*') {
            ...
            if (d > 0)
                issue('unterminated_comment', 'Block comment is not closed.', commentStart, n);
            continue;
        }
        var start = i, type = 'op';
        ...
        if (dq) { /* $$ … $$ dollar quote */
            ...
            i++;
            while (i < n) {
                if (escapeBackslash && sql[i] === '\\' && i + 1 < n) {
                    i += 2;
                    continue;
                }
                if (sql[i] === "'") {
            ...
            if (!stringClosed)
                issue('unterminated_string', 'String literal is not closed.', start, n);
            type = 'str';
        }
        else if (c === '[') {
            ...
            if (!bracketClosed)
                issue('unterminated_identifier', 'Bracketed identifier is not closed.', start, n);
            type = 'word';
        }
        else if (c === '"') {
            ...
            if (!quoteClosed)
                issue('unterminated_identifier', 'Quoted identifier is not closed.', start, n);
            type = 'word';
        }
        else if (c === '`') {
            var tickClosed = false;
            i++;
            while (i < n) {
                if (sql[i] === '`' && sql[i + 1] === '`') {
                    i += 2;
            ...
            if (!tickClosed)
                issue('unterminated_identifier', 'Backtick identifier is not closed.', start, n);
            type = 'word';
        }
        else if (/[A-Za-z_@#:]/.test(c) && !(c === ':' && sql[i + 1] !== ':')) {
            if (c === ':')
                i += 2;
            while (i < n && /[A-Za-z_@#$0-9]/.test(sql[i]))
                i++;
    ...
    toks.forEach(function (t) {
        if (t.v === '(')
            ...
            if (parens.length)
                ...
                diagnostics.push({ severity: 'error', code: 'unexpected_closing_parenthesis',
