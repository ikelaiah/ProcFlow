...
(function () {
    ...
    try {
        ...
        record('v2 future workspace versions are rejected without coercion', future.snapshot === null && future.migrated === false &&
        ...
        record('v2 future workspace versions are rejected without coercion', false, String(err && err.stack || err));
    }
    ...
    try {
        ...
        record('v1.12.0 report definition round-trips in a saved workspace', !!parsedR.snapshot && !parsedR.error &&
            parsedR.snapshot.report === reportText &&
            parsedR.snapshot.catalogue === 'dbo.a TABLE' &&
            !!migratedLegacyReport.snapshot &&
            migratedLegacyReport.snapshot.report === null, { report: parsedR.snapshot && parsedR.snapshot.report,
            legacyReport: migratedLegacyReport.snapshot && migratedLegacyReport.snapshot.report });
    }
    catch (err) {
        record('v1.12.0 report definition round-trips in a saved workspace', false, String(err && err.stack || err));
    }
    ...
    try {
        ...
        if (storageAvailable) {
            ...
            record('v2 future saved workspaces remain untouched after rejection', rejectedFuture === null && workspaceLastError() === 'future_workspace_version' &&
                futureStillStored === futureStored, { error: workspaceLastError(), preserved: futureStillStored === futureStored });
            clearWorkspace();
            ...
            record('v2 corrupt saved workspaces remain untouched after rejection', rejectedCorrupt === null && workspaceLastError() === 'corrupt_json' &&
                corruptStillStored === corruptStored, { error: workspaceLastError(), preserved: corruptStillStored === corruptStored });
            clearWorkspace();
        }
        else {
        ...
        record('v2 future saved workspaces remain untouched after rejection', false, String(err && err.stack || err));
        record('v2 corrupt saved workspaces remain untouched after rejection', false, String(err && err.stack || err));
    }
