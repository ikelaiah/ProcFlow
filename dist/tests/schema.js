...
(function () {
    ...
    function recordMermaid(name, pass, detail) {
        mermaidTotal++;
        if (pass)
            mermaidPassed++;
    ...
    function relationship(result, fromId, toId) {
    ...
    try {
        ...
        record('v2.1.0 T-SQL tables, composite PK, and declared FK relationships', t.stats.tables === 3 && t.stats.views === 0 && t.stats.columns === 7 &&
            ...
            !t.diagnostics.some(function (d) { return d.severity === 'error'; }), { stats: t.stats, diagnostics: t.diagnostics, keys: entity(t, 'DBO.ENROLLMENT') });
        record('v2.1.0 T-SQL identity, NOT NULL, and composite column flags', !!column(t, 'DBO.STUDENT', 'StudentId') && column(t, 'DBO.STUDENT', 'StudentId').identity === true &&
            ...
            column(t, 'DBO.STUDENT', 'Name').type === 'NVARCHAR(100)', entity(t, 'DBO.STUDENT'));
    ...
    try {
        ...
        record('v2.1.0 PostgreSQL quoted identifiers, serial, and unique-index one-to-one', p.stats.tables === 2 && p.stats.relationships === 1 &&
            ...
            !!key(p, 'PUBLIC.ORDER', 'unique', ['Customer Id']), { stats: p.stats, rel: orderRel, entity: entity(p, 'PUBLIC.ORDER') });
        record('v2.1.0 PostgreSQL raw type fidelity and generated columns', column(p, 'PUBLIC.ORDER', 'placed_at').type === 'timestamp with time zone' &&
            column(p, 'PUBLIC.ORDER', 'total').type === 'numeric(12,2)' &&
            column(p, 'PUBLIC.ORDER', 'total').generated === true &&
            column(p, 'PUBLIC.ORDER', 'total').computed === false, entity(p, 'PUBLIC.ORDER'));
    }
    catch (err) {
    ...
    try {
        ...
        record('v2.1.0 SQLite AUTOINCREMENT, inline REFERENCES, and raw affinity text', s.stats.tables === 2 && s.stats.relationships === 1 &&
            ...
            ownerRel.fromColumns.join(',') === 'id' && ownerRel.toColumns.join(',') === 'owner_id', { stats: s.stats, rel: ownerRel, entity: entity(s, 'GADGET') });
    }
    catch (err) {
    ...
    try {
        ...
        record('v2.1.0 DB2 identity, DECIMAL precision, and explicit unresolved target', d.stats.tables === 1 && d.stats.unresolved === 1 && d.stats.relationships === 1 &&
            ...
            entity(d, 'SALES').unresolved === false, { stats: d.stats, rel: regionRel, diagnostics: d.diagnostics });
    }
    catch (err) {
    ...
    try {
        ...
        record('v2.1.0 views keep declared columns and report unknown ones', v.stats.tables === 0 && v.stats.views === 2 &&
            !!known && known.kind === 'view' && known.columns.length === 2 && known.keys.length === 0 &&
            !!unknown && unknown.columns.length === 0 &&
            v.diagnostics.some(function (x) { return x.code === 'schema_columns_unknown'; }), { stats: v.stats, diagnostics: v.diagnostics });
    }
    catch (err) {
    ...
    try {
        ...
        record('v2.1.0 duplicate entities and unknown ALTER targets are reported, not merged', dup.entities.length === 1 &&
            dup.diagnostics.some(function (x) { return x.code === 'schema_duplicate_entity'; }) &&
            dup.diagnostics.some(function (x) { return x.code === 'schema_alter_unknown_table'; }), { entities: dup.entities, diagnostics: dup.diagnostics });
        ...
        record('v2.1.0 ambiguous FK targets are never guessed into an edge', ambiguous.relationships.length === 0 &&
            ambiguous.diagnostics.some(function (x) { return x.code === 'schema_ambiguous_reference'; }), { relationships: ambiguous.relationships, diagnostics: ambiguous.diagnostics });
        var heuristic = parseSchema([
            'CREATE TABLE s1.users (id INT PRIMARY KEY);',
            'CREATE TABLE t (id INT PRIMARY KEY, user_id INT REFERENCES users (id));'
        ].join('\n'));
        var heuristicRel = heuristic.relationships[0];
        record('v2.1.0 unqualified FK targets resolve by name with a labelled warning', !!heuristicRel && heuristicRel.resolution === 'heuristic' &&
            heuristic.diagnostics.some(function (x) { return x.code === 'schema_unresolved_reference'; }), { relationships: heuristic.relationships, diagnostics: heuristic.diagnostics });
        var empty = parseSchema('');
        record('v2.1.0 empty DDL is reported, not treated as a schema', empty.entities.length === 0 && empty.stats.relationships === 0 &&
            empty.diagnostics.some(function (x) { return x.code === 'schema_empty'; }), empty);
    }
    catch (err) {
    ...
    try {
        ...
        record('v2.1.0 DB2 ERD sample parses 8 tables, 4 views, and 6 relationships', db2Sample.stats.tables === 8 && db2Sample.stats.views === 4 &&
            ...
            !db2Sample.diagnostics.some(function (x) { return x.severity === 'error'; }), { stats: db2Sample.stats, diagnostics: db2Sample.diagnostics });
    }
    catch (err) {
    ...
    try {
        ...
        record('v2.1.0 bulk DDL estate parses without silent drops', bulkResult.stats.tables === 300 && bulkResult.stats.relationships === 300 &&
            bulkResult.stats.columns === 600 && bulkResult.stats.unresolved === 0 &&
            !bulkResult.diagnostics.some(function (x) { return x.severity !== 'info'; }) &&
            spansValid(bulkResult), bulkResult.stats);
    }
    catch (err) {
    ...
    try {
        ...
        record('v2.2.0 auto layout is deterministic and places every entity', JSON.stringify(autoLayout) === JSON.stringify(autoLayoutAgain) &&
        ...
        record('v2.2.0 auto layout orders parents before children without overlaps', overlaps === 0 &&
            layoutColumnOf['DBO.STUDENT'] < layoutColumnOf['DBO.ENROLLMENT'] &&
            layoutColumnOf['DBO.COURSE'] < layoutColumnOf['DBO.ENROLLMENT'], { columns: autoLayout.columns, overlaps: overlaps, positions: autoLayout.positions });
        var defaultLayout = erdDefaultLayout(layoutSchema, layoutSizes);
        record('v2.2.0 default layout preserves declaration order', defaultLayout.positions['DBO.STUDENT'].x < defaultLayout.positions['DBO.COURSE'].x &&
            defaultLayout.positions['DBO.STUDENT'].x === defaultLayout.positions['DBO.ENROLLMENT'].x, defaultLayout.positions);
    }
    catch (err) {
    ...
    try {
        ...
        record('v2.2.0 view bodies declare layout sources and place views downstream', !!orderSummary && !!orderSummary.sources &&
            ...
            viewColumnOf['SALES.AUDIT_LOG'] < viewColumnOf['SALES.V_AUDIT_RECENT'], { sources: orderSummary && orderSummary.sources, columns: viewLayout.columns });
    }
    catch (err) {
    ...
    try {
        ...
        record('v2.2.0 layout file round-trips with a schema fingerprint', roundTrips &&
        ...
        record('v2.2.0 layout parser rejects foreign, stale, and malformed files', !badFormat.file && !badVersion.file && !badJson.file &&
            ...
            badJson.diagnostics[0].code === 'erd_layout_parse_error', { badFormat: badFormat, badVersion: badVersion, badJson: badJson });
    }
    catch (err) {
    ...
    try {
        ...
        for (var bulkLayoutIndex = 1; bulkLayoutIndex <= 300; bulkLayoutIndex++) {
            bulkLayoutLines.push('CREATE TABLE app.t' + bulkLayoutIndex + ' (', '  id INT NOT NULL,', '  ref_id INT,', '  CONSTRAINT pk_t' + bulkLayoutIndex + ' PRIMARY KEY (id),', '  CONSTRAINT fk_t' + bulkLayoutIndex + ' FOREIGN KEY (ref_id) REFERENCES app.t' +
        ...
        record('v2.2.0 auto layout stays deterministic and overlap-free at 300 tables', placedIds.length === 300 && bulkLayoutOverlap === 0 &&
            bulkLayout.width > 0 && bulkLayout.height > 0, { placed: placedIds.length, overlap: bulkLayoutOverlap, columns: bulkLayout.columns.length });
    }
    catch (err) {
    ...
    try {
        ...
        recordMermaid('mermaid erDiagram declares entities with PK/FK/UK attributes', tm.indexOf('erDiagram') === 0 &&
            tm.indexOf('DBO_STUDENT {') >= 0 &&
            tm.indexOf('INT StudentId PK') >= 0 &&
            tm.indexOf('INT MentorId FK') >= 0 &&
            /DBO_ENROLLMENT \{\n(\s+.*\n)*?\s+\}/.test(tm) &&
            tm.indexOf('StudentId PK, FK') >= 0 &&
            tm.indexOf('Name UK') >= 0, tm);
        recordMermaid('mermaid relationships encode declared cardinality and optionality', tm.indexOf('DBO_STUDENT o|--o{ DBO_STUDENT : FK_Student_Mentor') >= 0 &&
            tm.indexOf('DBO_STUDENT ||--o{ DBO_ENROLLMENT') >= 0 &&
            tm.indexOf('DBO_COURSE ||--o{ DBO_ENROLLMENT') >= 0, tm);
        recordMermaid('mermaid provenance comments preserve original names and raw types', tm.indexOf('%% entity DBO_STUDENT = "dbo.Student" (table)') >= 0 &&
            tm.indexOf('%% column DBO_STUDENT.Name : NVARCHAR(100)') >= 0 &&
            tm.indexOf('%% relationship DBO_STUDENT -> DBO_ENROLLMENT') >= 0, tm);
        var pg = toMermaidER(parseSchema(PGSQL));
        recordMermaid('mermaid sanitizes identifiers and types, keeping raw text in a comment', pg.indexOf('timestamp_with_time_zone placed_at "timestamp with time zone"') >= 0 &&
            pg.indexOf('Order_Id') >= 0 &&
            pg.indexOf('PUBLIC_CUSTOMER ||--|| PUBLIC_ORDER') >= 0 &&
            pg.indexOf('PUBLIC_ORDER {') >= 0, pg);
        var db2 = toMermaidER(parseSchema(DB2));
        recordMermaid('mermaid exports unresolved targets as explicit external entities', db2.indexOf('EXTERNAL_REGION') >= 0 &&
            /EXTERNAL_REGION o\|--o\{ SALES/.test(db2) &&
            db2.indexOf('%% entity EXTERNAL_REGION = "region"') >= 0, db2);
        ...
        recordMermaid('mermaid entity identifiers are deterministic and collision-free', idLines.length === 2 && idLines[0].trim() !== idLines[1].trim() &&
            idLines[0].trim() === 'A_B {' && idLines[1].trim() === 'A_B_2 {', collisions);
        recordMermaid('mermaid export is deterministic', tm === toMermaidER(parseSchema(TSQL)), tm);
    }
    catch (err) {
