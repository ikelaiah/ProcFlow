...
(function () {
    ...
    frame.addEventListener('load', function () {
        ...
        (async function () {
            ...
            var sql = function () { return get('qb-sql-out').textContent || ''; };
            var pick = function (entity, column) {
                var box = d.querySelector('#erd-cards .erd-card[data-entity-id="' +
                    entity + '"] input.qb-pick[data-column="' + column + '"]');
                if (!box)
                    return false;
                box.click();
                return true;
            };
            var rowPick = function (entity, column) {
                var row = d.querySelector('#erd-cards .erd-card[data-entity-id="' +
                    entity + '"] li[data-column="' + column + '"]');
                if (!row)
                    return false;
                row.click();
                return true;
            ...
            await wait(250);
            /* ---- query mode ---- */
            record('query button exists and starts unpressed', !!get('btn-erd-query') &&
            ...
            await wait(250);
            record('query mode adds checkboxes and opens the window', get('btn-erd-query').getAttribute('aria-pressed') === 'true' &&
            ...
            record('picked-column chip renders', d.querySelectorAll('#qb-picks .qb-chip').length === 1);
            ...
            var joinSelect = d.querySelector('#qb-plan-body select[data-role="join-type"]');
            ...
            var limitSelect = get('qb-limit');
            limitSelect.value = '100';
            limitSelect.dispatchEvent(new Event('change'));
            record('row cap reaches the SQL', sql().indexOf('TOP 100') >= 0, sql());
            d.querySelector('#qb-picks .qb-chip-sort').click();
            record('sort cycles to ascending', sql().indexOf('ORDER BY') >= 0 && sql().indexOf(' ASC') >= 0, sql());
            d.querySelector('#qb-picks .qb-chip-sort').click();
            record('sort cycles to descending', sql().indexOf(' DESC') >= 0);
            d.querySelector('#qb-picks .qb-chip-sort').click();
            record('sort cycles off', sql().indexOf('ORDER BY') < 0);
            record('SQL is syntax highlighted', (d.querySelector('#qb-sql-out .sql-kw') || { textContent: '' }).textContent === 'SELECT' &&
            ...
            var targetAgg = chipAggs[1];
            targetAgg.value = 'count';
            targetAgg.dispatchEvent(new Event('change', { bubbles: true }));
            await wait(120);
            record('aggregate renders a function and GROUP BY', sql().indexOf('COUNT(orderheader.[PlacedAt]) AS count_orderheader_placedat') >= 0 &&
                sql().indexOf('GROUP BY customer.[Email]') >= 0, sql());
            record('distinct is disabled and ignored with aggregates', get('qb-distinct').disabled === true &&
                sql().indexOf('SELECT DISTINCT') < 0);
            record('aggregate tip explains row multiplication', (d.querySelector('#qb-plan-body .qb-tip') || { textContent: '' })
                .textContent.indexOf('Aggregates collapse grouped rows') >= 0);
            var againAgg = d.querySelectorAll('#qb-picks .qb-chip-agg')[1];
            againAgg.value = 'count-distinct';
            againAgg.dispatchEvent(new Event('change', { bubbles: true }));
            await wait(100);
            record('count distinct renders COUNT(DISTINCT ...)', sql().indexOf('COUNT(DISTINCT orderheader.[PlacedAt])') >= 0, sql());
            get('btn-qb-clear').click();
            await wait(80);
            record('clear resets aggregate selections', d.querySelectorAll('#qb-picks .qb-chip-agg').length === 0 &&
            ...
            record('pointer resize grows the window', grownRect.width > floatRect.width + 50 && grownRect.height > floatRect.height + 30);
            grip.focus();
            grip.dispatchEvent(new KeyboardEvent('keydown', { key: 'ArrowLeft', bubbles: true }));
            record('keyboard resize shrinks the window', get('qb-float').getBoundingClientRect().width < grownRect.width);
            /* ---- disconnected problem and resolutions ---- */
            record('picking an unjoinable table works', pick('DBO.VORDERVALUE', 'OrderId'));
            await wait(250);
            record('problem card and coral ring appear', !!d.querySelector('#qb-plan-body .qb-problem') &&
                !!d.querySelector('#erd-cards .erd-card[data-entity-id="DBO.VORDERVALUE"].qb-problem-card'));
            record('unjoined picks stay out of the SQL', sql().indexOf('vordervalue') < 0, sql());
            record('teaching mode starts from the problem card', !!d.querySelector('#qb-plan-body [data-action="teach-click"]'));
            d.querySelector('#qb-plan-body [data-action="teach-click"]').click();
            record('teach banner shows', !!d.querySelector('#qb-plan-body .qb-teach-banner'));
            rowPick('DBO.CUSTOMER', 'Email');
            record('first taught column is marked', !!d.querySelector('#erd-cards li.qb-teach-first'));
            rowPick('DBO.VORDERVALUE', 'OrderId');
            await wait(200);
            record('taught join appears as not declared', sql().indexOf('ON customer.[Email] = vordervalue.[OrderId]') >= 0 &&
                sql().indexOf('taught join') >= 0, sql());
            record('taught join clears the problem', !d.querySelector('#qb-plan-body .qb-problem'));
            /* ---- self join ---- */
            get('btn-qb-clear').click();
            ...
            await wait(200);
            record('self join adds a second alias', sql().indexOf('INNER JOIN [dbo].[Customer] AS customer_2') >= 0 &&
                sql().indexOf('ON customer.[ReferredBy] = customer_2.[CustomerId]') >= 0, sql());
            var copyBox = d.querySelector('#qb-plan-body input[data-role="copy-column"][data-column="DisplayName"]');
            ...
            await wait(150);
            record('copy columns route to the second alias', sql().indexOf('customer_2.[DisplayName]') >= 0 &&
                sql().indexOf('second copy, self join: DisplayName') >= 0, sql());
            /* ---- escape and clear ---- */
            get('btn-qb-teach').click();
            record('teach toggle is pressed', get('btn-qb-teach').getAttribute('aria-pressed') === 'true');
            d.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', bubbles: true }));
            record('escape cancels teaching', get('btn-qb-teach').getAttribute('aria-pressed') === 'false');
            get('btn-qb-clear').click();
            record('clear resets picks and chips', d.querySelectorAll('#qb-picks .qb-chip').length === 0 &&
                sql().indexOf('Pick columns on the diagram') >= 0);
            /* ---- dialect, comments, copy state ---- */
            pick('DBO.CUSTOMER', 'Email');
            await wait(100);
            var dialectSelect = get('qb-dialect');
            dialectSelect.value = 'postgres';
            dialectSelect.dispatchEvent(new Event('change'));
            record('dialect switch requotes identifiers', sql().indexOf('"dbo"."Customer"') >= 0, sql());
            dialectSelect.value = 'tsql';
            dialectSelect.dispatchEvent(new Event('change'));
            get('qb-comments').checked = false;
            get('qb-comments').dispatchEvent(new Event('change'));
            record('comments toggle strips the provenance header', sql().indexOf('/*') < 0 && sql().indexOf('SELECT') === 0, sql());
            get('qb-comments').checked = true;
            get('qb-comments').dispatchEvent(new Event('change'));
            /* ---- find and only-used filter ---- */
            var findBox = get('erd-find');
            findBox.value = 'Email';
            findBox.dispatchEvent(new Event('input'));
            record('find matches columns and marks the rows', d.querySelectorAll('#erd-cards li.col-match').length >= 1 &&
            ...
            pick('DBO.ORDERHEADER', 'PlacedAt');
            await wait(150);
            var onlyUsedBox = get('qb-only-used');
            onlyUsedBox.checked = true;
            onlyUsedBox.dispatchEvent(new Event('change'));
            var productCard = d.querySelector('#erd-cards .erd-card[data-entity-id="DBO.PRODUCT"]');
            record('only-used filter hides unused tables', !!productCard && productCard.offsetParent === null);
            record('only-used filter keeps query tables', d.querySelector('#erd-cards .erd-card[data-entity-id="DBO.CUSTOMER"]')
                .offsetParent !== null);
            ...
            get('btn-qb-close').click();
            await wait(200);
            record('closing resets mode, cards, and compact', get('qb-float').hidden === true &&
                get('btn-erd-query').getAttribute('aria-pressed') === 'false' &&
                d.querySelectorAll('#erd-cards input.qb-pick').length === 0 &&
                get('erd-compact').disabled === false);
            get('btn-erd-query').click();
            await wait(200);
            record('reopening keeps picks and rebuilds SQL', d.querySelectorAll('#qb-picks .qb-chip').length === 2 &&
