...
(function () {
    ...
    PROCFLOW_RANGE_FIXTURES.forEach(function (rangeFixture) {
        try {
            ...
            record(rangeFixture.name + ' · statement ranges', sameRanges && rangesInBounds && rangeDiag, JSON.stringify({ expected: rangeFixture.statements, actual: rangeTexts,
    ...
    try {
        ...
        var preparedResult = analyse(preparedDb2, { dialect: 'db2', mode: 'flow', group: false, sources: true });
        var preparedEstate = analyseEstate([{ name: 'prepared.sql', text: preparedDb2 }], { dialect: 'db2', mode: 'auto', group: false, sources: true });
        var preparedObject = preparedEstate.objects[0];
        record('DB2 prepared SQL stays opaque and never becomes object dependencies', !!preparedObject && preparedObject.reads.length === 0 && preparedObject.calls.length === 0 &&
            preparedResult.diagnostics.some(function (d) { return d.code === 'dynamic_sql'; }) &&
            preparedResult.graph.nodes.some(function (n) {
                return n.cls === 'opaque' &&
                    n.text.indexOf('EXECUTE stmt') >= 0;
            }), JSON.stringify({ analysis: preparedResult, estate: preparedEstate }));
    }
    catch (err) {
        record('DB2 prepared SQL stays opaque and never becomes object dependencies', false, String(err && err.stack || err));
    }
    try {
    ...
    try {
        ...
        record('v1.2.0 number lexing (0x, separators, 1., .5)', numberLexing && dottedName, JSON.stringify(nt.map(function (t) { return t.v + '[' + t.type + ']'; })));
    }
    catch (err) {
        record('v1.2.0 number lexing (0x, separators, 1., .5)', false, String(err && err.stack || err));
    }
    try {
        ...
        record('v1.2.0 dialect_ambiguous guardrail on low-confidence tie', ambiguous, JSON.stringify({ detected: amb.detected, diagnostics: amb.diagnostics }));
    }
    catch (err) {
        record('v1.2.0 dialect_ambiguous guardrail on low-confidence tie', false, String(err && err.stack || err));
    }
    try {
        ...
        var missingEnd = analyse('CREATE PROC dbo.bad AS BEGIN SELECT 1;', { dialect: 'tsql', mode: 'auto', group: false, sources: true });
        ...
        record('v1.2.0 bracket and BEGIN/END balance diagnostics', hasParenDiag && hasStrayDiag && hasMissingDiag, JSON.stringify({ unbalanced: unbalanced.diagnostics,
            strayEnd: strayEnd.diagnostics,
            missingEnd: missingEnd.diagnostics }));
    }
    catch (err) {
        record('v1.2.0 bracket and BEGIN/END balance diagnostics', false, String(err && err.stack || err));
    }
    try {
        var headerSql = 'CREATE VIEW dbo.schemabound WITH SCHEMABINDING, VIEW_METADATA AS SELECT id FROM dbo.t;';
        var headerWith = analyse(headerSql, { dialect: 'tsql', mode: 'auto', group: false, sources: true });
        var irWith = buildObjectIR(headerWith, { id: 'f', name: headerWith.header.name, kind: headerWith.header.kind, file: 'f', sql: headerSql });
        ...
        var noGoSource = 'CREATE PROC dbo.first_obj AS BEGIN SELECT 1; END\n' +
            'CREATE PROC dbo.second_obj AS BEGIN SELECT 2; END';
        var noGoEstate = analyseEstate([{ name: 'no-go.sql', text: noGoSource }], { dialect: 'tsql', mode: 'auto', group: false, sources: true });
        ...
        record('v1.2.0 findBody: CREATE VIEW … WITH header', viewHeaderOk, JSON.stringify({ header: headerWith.header, ir: irWith }));
        record('v1.2.0 findBody: ALTER object headers', altHeaderOk, JSON.stringify(altPlans));
        record('v1.2.0 findBody: multi-object script without GO', noGoOk, JSON.stringify(noGoEstate.objects.map(function (o) { return o.name; })));
    }
    catch (err) {
        record('v1.2.0 findBody: CREATE VIEW … WITH header', false, String(err && err.stack || err));
    ...
    try {
        ...
        record('v1.4.0 APPLY and tabular functions as structured references', false, String(err && err.stack || err));
    }
    try {
        var nodeTexts40 = function (sql, dialect) {
            var a = analyse(sql, { dialect: dialect, mode: 'query', group: false, sources: true });
        ...
        record('v1.4.0 read extraction MERGE…USING / DELETE…USING / UPDATE…FROM', readOk40, JSON.stringify({ merge: merge40, del: del40, upd: upd40 }));
    }
    catch (err) {
        record('v1.4.0 read extraction MERGE…USING / DELETE…USING / UPDATE…FROM', false, String(err && err.stack || err));
    }
    try {
        ...
        var recMarked40 = rec40.graph.nodes.some(function (n) {
            return /recursive CTE/.test(n.text);
        ...
        record('v1.4.0 recursive CTE informational annotation and metadata', false, String(err && err.stack || err));
        record('v1.4.0 approximate recursion emits a warning', false, String(err && err.stack || err));
    }
    try {
        var derived40 = analyse('SELECT x.id FROM (SELECT id FROM dbo.inner) x;', { dialect: 'tsql', mode: 'query', group: false, sources: true });
        var commaQ40 = analyse('SELECT * FROM dbo.a, dbo.b;', { dialect: 'tsql', mode: 'query', group: false, sources: true });
        ...
        record('v1.4.0 derived-table and comma inner sources wired in query graph', derivedOk40 && commaQOk40, JSON.stringify({ derived: derived40.graph.nodes.map(function (n) { return n.text; }),
            comma: commaQ40.graph.nodes.map(function (n) { return n.text; }) }));
    }
    catch (err) {
        record('v1.4.0 derived-table and comma inner sources wired in query graph', false, String(err && err.stack || err));
    }
    try {
        var qgSrc40 = [
        ...
        var qg40 = analyse(qgSrc40, { dialect: 'tsql', mode: 'query', group: false, sources: true });
    ...
    try {
        ...
        var broken160 = analyse('END SELECT (1;', { dialect: 'tsql', mode: 'auto', group: false, sources: true });
        ...
        record('v1.6.0 versioned confidence formula from per-region signals', cleanOk160 && opaqueOk160 && brokenOk160, JSON.stringify({ clean: cleanSig160, opaque: opaque160.confidenceSignals,
            broken: broken160.confidenceSignals,
            confidences: [clean160.confidence, opaque160.confidence, broken160.confidence] }));
    }
    catch (err) {
        record('v1.6.0 versioned confidence formula from per-region signals', false, String(err && err.stack || err));
    }
    try {
        ...
        record('v1.6.0 document-scoped findings carry no fabricated span', noFabricatedSpan160 && aroundOk160, JSON.stringify(docScoped160.map(function (d) { return { code: d.code, scope: d.scope, span: d.span }; })));
    }
    catch (err) {
        record('v1.6.0 document-scoped findings carry no fabricated span', false, String(err && err.stack || err));
    }
    try {
        var opTable160 = analyse('SELECT * FROM dbo.doc d, UNNEST(d.ids) AS x(id);', { dialect: 'plpgsql', mode: 'query', group: false, sources: true });
        var apply160 = analyse('SELECT a.id FROM dbo.a a CROSS APPLY dbo.fn(a.id) f;', { dialect: 'tsql', mode: 'query', group: false, sources: true });
        ...
        record('v1.6.0 partially resolved APPLY region diagnostic', false, String(err && err.stack || err));
    }
    try {
