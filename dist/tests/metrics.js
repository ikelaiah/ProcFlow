...
(function () {
    ...
    corpus.forEach(function (f) {
        ...
        if (r.confidenceFormulaVersion)
            sampleConfidenceFormula = r.confidenceFormulaVersion;
        r.diagnostics.forEach(function (d) {
            if (d.scope === 'region') {
                regionScoped++;
                if (!!d.span && d.span.start >= 0 && d.span.end > d.span.start)
                    regionValidSpan++;
            }
        });
        r.graph.edges.forEach(function (e) { edges++; if (!!e.kind)
            kindEdges++; });
        r.graph.nodes.forEach(function (n) { nodes++; if (!!n.provenance)
            provenanceNodes++; });
        walk(r.ast);
    });
    function rate(n, d) {
    ...
    var metrics = {
        ...
        exportTraceabilityRate: window.PROCFLOW_PARITY_RESULT
            ? rate(window.PROCFLOW_PARITY_RESULT.traceabilityPassed, window.PROCFLOW_PARITY_RESULT.traceabilityTotal)
            : 1,
