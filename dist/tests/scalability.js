"use strict";
/* v2.0.0 deterministic scale and realistic-corpus invariants. Timing is
...
(function () {
    ...
    function stressSql(size) {
        var filler = new Array(size + 1).join(' ');
        return 'SELECT 1;\n' + filler;
    }
    function checkAnalysis(sql) {
    ...
    var files = [{ name: 'scale-100.sql', text: (function () {
                var parts = [];
                for (var i = 1; i <= 100; i++)
    ...
    record('multi-object estate · approximately 100 SQL objects', estate.objects.length === 100 &&
        estate.graph.nodes.length >= 100 && estate.graph.edges.length >= 100, { objects: estate.objects.length, nodes: estate.graph.nodes.length, edges: estate.graph.edges.length });
    var datasetParts = [];
    for (var d = 1; d <= 25; d++)
        datasetParts.push('<DataSet Name="Dataset' + d + '"><Query><CommandText>' +
    ...
    record('report dependency graph · approximately 25 datasets', report.datasetCount === 25 && report.embeddedCount === 25 &&
        reportGraph.stats.datasets === 25 && reportGraph.stats.objects >= 25, { datasets: report.datasetCount, embedded: report.embeddedCount,
        graphStats: reportGraph.stats });
    function scaleGraph(count) {
        var nodes = [];
        var edges = [];
        for (var i = 0; i < count; i++)
            nodes.push({ id: 'scale-' + i, shape: 'rect',
                text: 'Scale node ' + i, cls: 'stmt', source: null, provenance: 'synthetic' });
        for (var e = 1; e < count; e++)
            edges.push({ from: 'scale-' + (e - 1), to: 'scale-' + e,
    ...
    [100, 250, 500].forEach(function (count) {
        ...
        record('dependency layout budget · ' + count + ' nodes', Object.keys(positions).length === count && layout.overlaps === 0 &&
            layout.backboneEdges === count - 1 && !parsed.querySelector('parsererror') &&
            toMermaid(graph, 'TD').indexOf('flowchart TD') === 0, { nodes: count, overlaps: layout.overlaps, crossings: layout.crossings,
    ...
    if (summary)
        summary.textContent = 'v2.0.0 scale/corpus · ' + passed + '/' + results.length;
})();
